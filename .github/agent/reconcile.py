...
new_predicate = '''    const auto payable_one_discard_regi_route = [this](const Card item) {
      return hand_count(item) > 0 &&
             choose_discard(true, false, true, item).has_value();
    };
...
if start == -1 or end == -1 or end <= start:
    if "test_quick_ball_only_route_preserves_dialga" not in text:
        raise SystemExit("issue 653 positive-test anchor missing")
else:
    replacement = r'''void test_quick_ball_only_route_preserves_dialga() {
  ...
  state.hand = {sim::Card::TapuLeleGX, sim::Card::DialgaGX,
                ...
                sim::Card::MegaDragonite, sim::Card::ChaoticSwell,
                sim::Card::Powerglass};
  sim::EngineTestAccess::set_state(engine, std::move(state));

  // Quick Ball's printed one-card discard cost is payable with the second Dragon
  // payload, so it publicly exposes Regidrago V while Dialga-GX remains in hand:
...
void test_mysterious_treasure_only_route_preserves_dialga() {
  ...
  // Mysterious Treasure's printed one-card discard cost is payable with the second
  // Dragon payload, and Regidrago V is a legal Dragon search target:
...
text = text.replace(
    ...
    "  test_quick_ball_only_route_preserves_dialga();\n"
    "  test_mysterious_treasure_only_route_preserves_dialga();\n",
    1,
)
