...
`src/trace_engine_v2/composition/engine_body.inc` is the canonical Engine composition owner. `composition/opening_engine_overrides.inc` owns the early Supporter/VSTAR continuation. `composition/post_014a_overrides.inc` owns late-search composition. `composition/opening_legacy_stage.inc` owns the historical `part_003.inc` through `part_005.inc` opening chain and its temporary aliases. `composition/middle_legacy_stage.inc` owns the banked-Tapu continuation, shared lock-removal composition, and the temporary aliases spanning `opening_engine_overrides.inc`, `turn_action_policy_runtime.inc`, and `post_014a_overrides.inc`. `composition/late_legacy_stage.inc` owns the historical `part_014c.inc` through `part_016.inc` execution, registry, reporting chain and its cross-fragment aliases.
...
Completed in `cleanup-1786780977699`: extracted the full middle banked-Tapu and lock-removal composition block from `composition/engine_body.inc` into `composition/middle_legacy_stage.inc`. The stage preserves the established macro lifetime, local entry/exit assertions, include roots, and member boundaries around `opening_engine_overrides.inc`, `turn_action_policy_runtime.inc`, `core/routes/banked_tapu_retreat_policy.inc`, `core/forest_field_blower_policy.inc`, and `post_014a_overrides.inc`. Route policy and gameplay resolution remain in their existing owners. C++ textual-include semantics: https://eel.is/c++draft/cpp.include

Next composition step: inspect the remaining composition-only forwarding inside `opening_engine_overrides.inc` and `post_014a_overrides.inc`. Extract only a boundary whose macro contract can be asserted locally, and keep strategy or printed card resolution in its current `core/` owner.
...
Next mechanical Forretress step: inventory the remaining direct Active/Bench scans and migrate only queries whose ordering and mutation semantics match an existing board-state seam exactly. Keep attachment distribution, self-Knock-Out resolution, promotion/retreat strategy ranks, DCI/UDP/AMR, K0/K1, readiness, and route policy in Forretress or their existing strategy owners. Forretress ex: https://api.pokemontcg.io/v2/cards/sv4pt5-2 Pineco: https://api.pokemontcg.io/v2/cards/sv4pt5-1 Forest of Vitality: https://api.pokemontcg.io/v2/cards/me1-117 Core evolution rules: https://www.pokemon.com/us/pokemon-tcg/rules Official February 2026 ruling: https://professorprogram.pokemon.com/news/11473085
