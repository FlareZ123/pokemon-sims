...
## Cleanup wave 2026-08-14 Forretress scenario composition follow-up

- `src/trace_engine_v2/part_forretress_ex_combo.inc` now owns the Garbodor scenario extension directly beside `core/forretress/runtime.inc` at the same proven namespace-scope boundary. The former `src/trace_engine_v2/core/forretress/scenarios.inc` one-owner fragment is retired. Composition owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/part_forretress_ex_combo.inc C++ textual-include semantics: https://eel.is/c++draft/cpp.include
- The inlined scenario block preserves the two scenario labels, declaration order, `ScenarioList` / `GarbodorScenarioSet` types, shared `append_scenarios()` helper, lookup helper, common-random-number row ordering, and direct source URLs. Garbodor: https://api.pokemontcg.io/v2/cards/xy9-57 Boost Shake: https://api.pokemontcg.io/v2/cards/swsh7-142 Scenario specification: https://github.com/FlareZ123/pokemon-sims/issues/2808
- Future Forretress cleanup should keep this namespace composition boundary thin and avoid recreating a one-owner scenario `.inc` fragment. Runtime behavior remains in `src/trace_engine_v2/core/forretress/runtime.inc`; DCI/UDP/AMR, K0/K1, lock behavior, action ordering, and readiness policy remain unchanged.
