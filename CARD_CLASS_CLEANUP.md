...
- The shared board-state seam keeps Active-first predicate short-circuiting explicit in `core/board_state_policy.inc`, while `core/forretress/contract.inc` owns the Forretress board index and attachment-destination contract. Keep future Forretress cleanup on those canonical seams rather than reintroducing raw index container types in forwarding `.inc` files. Board policy: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/board_state_policy.inc Forretress contract: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/forretress/contract.inc
...
- `src/trace_engine_v2/core/board_state_policy.inc` owns one Active-first `find_board_pokemon_matching()` traversal. Boolean board queries delegate to that finder, preserving the repository's explicit Active-before-Bench short-circuit contract while giving later cleanup one reusable board lookup seam. C++ algorithm semantics: https://eel.is/c++draft/alg.find
- `src/trace_engine_v2/core/forretress/contract.inc` owns `OptionalBoardIndex` beside `BoardIndex` and `AttachmentDestinations`, and marks pure combo queries `[[nodiscard]]`. This keeps board-index/result vocabulary in the canonical Forretress module rather than spreading raw index container types through forwarding `.inc` files. Pineco / Forretress ex sources: https://api.pokemontcg.io/v2/cards/sv4pt5-1 https://api.pokemontcg.io/v2/cards/sv4pt5-2
- Next safe follow-up: apply `BoardIndex`, `OptionalBoardIndex`, and `AttachmentDestinations` consistently inside the out-of-class definitions in `src/trace_engine_v2/core/forretress/runtime.inc`. Keep that future edit type-only, preserve Exploding Energy's existing card/ruling URLs beside its resolver, and do not combine it with route-policy changes. Current implementation: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/forretress/runtime.inc
...
## Cleanup wave 2026-08-14 Forretress contract cleanup

- The unreferenced `src/trace_engine_v2/core/forretress_combo_contract.inc` forwarding shim is retired. The single class-member contract remains `src/trace_engine_v2/core/forretress/contract.inc`: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/forretress/contract.inc
- The canonical contract now labels its ownership and shared board-index vocabulary directly, while `src/trace_engine_v2/core/board_state_policy.inc` keeps the Active-first generic board traversal visibly grouped. These are structure and layout changes only.
- The next type-only cleanup remains applying `BoardIndex`, `OptionalBoardIndex`, and `AttachmentDestinations` inside `src/trace_engine_v2/core/forretress/runtime.inc`. Preserve all existing Exploding Energy card, ruling, and policy URLs beside the resolver when that migration is performed. Runtime owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/forretress/runtime.inc
- This wave changes no card resolution, DCI/UDP/AMR, connector priority, K0/K1 state, lock behavior, or setup policy. C++ textual-include semantics: https://eel.is/c++draft/cpp.include
