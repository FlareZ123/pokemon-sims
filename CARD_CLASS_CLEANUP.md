...
The Treasure -> Tapu Lele-GX -> Steven route family now has a canonical semantic owner at `src/trace_engine_v2/core/routes/treasure_tapu_steven_policy.inc`. `part_issue_1356_fss_treasure_energy_override.inc` composes that owner directly at the same member boundary, while the duplicate root `part_issue_1411_treasure_tapu_steven_override.inc` path has been retired. The route body and its direct Mysterious Treasure, Tapu Lele-GX, Steven's Resolve, Crispin, Brilliant Blender, Regidrago, attachment, Supporter, lock, JIT, and hidden-information sources are byte-identical to the previous fragment; only semantic ownership and the include path changed. Mysterious Treasure: https://api.pokemontcg.io/v2/cards/sm6-113 Tapu Lele-GX: https://api.pokemontcg.io/v2/cards/sm2-60 Steven's Resolve: https://api.pokemontcg.io/v2/cards/sm7-145 Canonical owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/treasure_tapu_steven_policy.inc

The issue-1393 held-Crispin completion helper now has a canonical semantic owner at `src/trace_engine_v2/core/routes/crispin_supported_route_policy.inc`. The historical root `part_issue_1393_crispin_route_helper.inc` path is reduced to a minimal compatibility composition seam that includes the canonical owner, preserving its member boundary while route code, DDE-aware projection, K1/JIT policy, and direct rules/card sources live together under `core/routes/`. Crispin: https://api.pokemontcg.io/v2/cards/sv7-133 Double Dragon Energy: https://api.pokemontcg.io/v2/cards/xy6-97 Advanced procedure: https://github.com/FlareZ123/pokemon-sims/blob/main/EN_advanced_manual-2025-transcription-structured.md Canonical owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/crispin_supported_route_policy.inc

The Forest Seal Stone route families now have canonical semantic owners at `core/routes/fss_target_policy.inc`, `core/routes/fss_oricorio_treasure_policy.inc`, and `core/routes/fss_blender_priority_policy.inc`. Their former root files remain minimal compatibility includes at the exact historical textual boundaries, so `fss_target_after_search_started_issue1071_original`, `fss_target_after_search_started_issue1735_original`, and the issue-1749 `in_play_count` macro lifetime keep the same declaration and preprocessing order. The canonical files retain the original direct Forest Seal Stone, Brilliant Blender, Oricorio, Trainer, Energy, K0/K1, DCI/JIT, and route-policy sources. Forest Seal Stone: https://api.pokemontcg.io/v2/cards/swsh12-156 Brilliant Blender: https://api.pokemontcg.io/v2/cards/sv8-164 Oricorio: https://api.pokemontcg.io/v2/cards/sm2-55 Canonical target owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/fss_target_policy.inc Canonical Oricorio owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/fss_oricorio_treasure_policy.inc Canonical Blender owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/fss_blender_priority_policy.inc

Next composition step: retire the held-Crispin compatibility seam and the three Forest Seal Stone compatibility includes only after every source-contract or generator consumer references the canonical `core/routes/` owners directly. For other remaining root `part_*` seams, move only a live complete macro lifetime or function body at the identical textual boundary. Preserve declaration order, route semantics, direct source URLs, and existing helper reuse. Do not recreate forwarding-only sequencers; compatibility includes are temporary migration seams, not new semantic owners.
...
The held-Crispin #1393 route now uses one projected Apex-energy evaluator for both one-Basic and two-Basic completion checks, so projection state changes cannot drift between those branches. The #2271 surplus-Regidrago route also names its K1 Tapu Lele-GX -> Arven -> Earthen Vessel -> Grass deck chain once before payload admission, keeping the route's access proof separate from its shared payload-zone query. Crispin: https://api.pokemontcg.io/v2/cards/sv7-133 Crispin ruling: https://compendium.pokegym.net/category/5-trainers/crispin/ Double Dragon Energy: https://www.pokemon.com/us/pokemon-tcg/pokemon-cards/series/xy6/97/ Mysterious Treasure: https://api.pokemontcg.io/v2/cards/sm6-113 Tapu Lele-GX: https://api.pokemontcg.io/v2/cards/sm2-60 Arven: https://api.pokemontcg.io/v2/cards/sv1-166 Earthen Vessel: https://api.pokemontcg.io/v2/cards/sv4-163 Regidrago VSTAR: https://api.pokemontcg.io/v2/cards/swsh12-136
...
Next payload step: replace remaining ad hoc Dragon-payload membership and cardinality scans only where semantics exactly match `PayloadZonePolicy::contains()`, `contains_card()`, `contains_matching()`, `count()`, or `count_matching()`. Preserve physical-order selection when order is observable and preserve the explicit strategic order where preference is required. Keep DCI/JIT predicates and discard timing at strategy owners. Within route-local projections, name repeated access chains and projected state transitions before considering a file move; move a complete root `part_*` function only when its declaration order and composition boundary are proven unchanged.
...
`core/routes/gladion_steven_route_policy.inc` now owns the shared `resolve_gladion_prize_exchange()` state transition used after a route has legally selected a revealed Prize target. The issue-1204 Treasure continuation plus the issue-2368 Treasure finish and issue-1595 Grass recovery delegate the physical Gladion exchange to that resolver while retaining their route admission, target choice, hidden-information sequencing, DCI/JIT policy, and trace text at the existing overlays. Gladion exact print: https://api.pokemontcg.io/v2/cards/sm4-95 Official Supporter and Prize procedure: https://www.pokemon.com/us/pokemon-tcg/rules Advanced procedure: https://github.com/FlareZ123/pokemon-sims/blob/main/EN_advanced_manual-2025-transcription-structured.md
...
Next Steven cleanup step: migrate other duplicated Gladion Prize-exchange mutations to `resolve_gladion_prize_exchange()` only when the caller has already completed legal reveal and target selection with identical semantics. Keep route admission, DCI/UDP/AMR, Supporter contention, connector domination, hidden-information sequencing, target ranking, and source URLs at their current owners. Continue retiring only composition-only `part_*steven*` forwarders whose canonical `core/routes/` owner can replace them at the identical textual boundary, and reuse `item_locked_on_turn()` for exact shared persistent Item-lock timing.
...
The Roseanne route now gives both copied-Engine admission projections named members, `roseanne_incense_then_vessel_is_payable()` and `roseanne_communication_then_vessel_is_payable()`, so the Supporter selector composes explicit semantic checks instead of rebuilding anonymous lambdas. The Earthen Vessel VSTAR window likewise centralizes its K0/K1 Basic Energy inventory through `earthen_vessel_known_or_unresolved_energy_count()`, while the Celestial Roar override names its Fire-first legal search transition through `search_celestial_roar_vessel_energy()`. These are behavior-preserving local ownership steps with the existing card, knowledge, JIT, and hidden-information sources kept beside the moved logic. Roseanne's Backup: https://api.pokemontcg.io/v2/cards/swsh9-148 Earthen Vessel: https://api.pokemontcg.io/v2/cards/sv4-163 Celestial Roar: https://api.pokemontcg.io/v2/cards/swsh12-135 K0/K1 policy: https://github.com/FlareZ123/pokemon-sims/blob/main/docs/POLICY_DECISIONS.md#knowledge-states

Next projection step: prefer named pure-projection members over route-local anonymous lambdas when the projection is reused or carries a distinct policy contract. Merge a remaining root fragment into a canonical semantic owner only when its complete function body or macro lifetime can move at the identical textual boundary. Keep physical resolution, trace emission, K0/K1 transitions, strategic route choice, and source URLs at their current owners.
