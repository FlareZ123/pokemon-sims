...
`src/trace_engine_v2/core/board_state_policy.inc` owns the shared Active-first mutable/const board traversal, prior-turn evolution timing predicate, canonical `BoardIndex` / `OptionalBoardIndex` vocabulary, shared `BoardAttachmentDestinations` storage, and pointer-to-index conversion. The board-index mapping is named by `kActiveBoardIndex`, `board_index_for_bench_position()`, and `bench_position_from_board_index()`. Mutable/const pointer lookup reuses the same index-discovery plus index-lookup path, while `board_index_for_pointer()` provides one shared conversion for an already-selected board Pokémon. Its internal zone-level helper is `board_index_for_pointer_in_zones()`, matching the public seam's vocabulary. `src/trace_engine_v2/core/forretress/contract.inc` consumes the shared attachment-destination storage through its card-facing `AttachmentDestinations` alias and owns the reusable `is_forretress_ex_board_pokemon()` / `is_regidrago_board_pokemon()` exact-print role predicates.
...
Completed in `cleanup-1786765850047`: the Forretress declaration contract now centralizes the repeated Forretress ex and Regidrago V/VSTAR board-role predicates with exact-print sources, and the internal pointer-to-index zone helper is named `board_index_for_pointer_in_zones()` to match the public `board_index_for_pointer()` seam. These are declaration/query cleanup changes only; no runtime admission, attachment selection, self-Knock-Out, promotion, retreat payment, DCI/UDP/AMR, K0/K1, readiness, or route policy moved.

Next mechanical Forretress step: consume `is_forretress_ex_board_pokemon()` and `is_regidrago_board_pokemon()` in the remaining Active-specific source/blocker questions inside `use_exploding_energy_for_setup()`, then assess whether the post-Ability retreat-target ranking can reuse a named board query without changing its deterministic ranking. Preserve selection, attachment distribution, self-Knock-Out, promotion, retreat payment, and route policy at the Forretress owner.
...
The issue-3202 Steven/VSTAR/Vessel policy now has one executable include path: `composition/opening_engine_overrides.inc` composes `core/routes/steven_vstar_vessel_route_policy.inc` directly at the same post-`part_010_steven_crispin_override.inc` member and macro boundary, and the historical `part_issue_3202_steven_vstar_vessel_semantic_override.inc` forwarder is retired. Canonical route owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/steven_vstar_vessel_route_policy.inc Composition owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/composition/opening_engine_overrides.inc Route specification: https://github.com/FlareZ123/pokemon-sims/issues/3202 C++ textual-include semantics: https://eel.is/c++draft/cpp.include

The issue-3203 Active-VSTAR Steven policy is composed directly from `core/routes/active_vstar_steven_route_policy.inc` by `part_issue_1067_arven_before_late_steven_override.inc` at the exact former forwarder boundary. CI exposed that nested parent include after the initial deletion probe, so the parent was retargeted before retiring `part_issue_3203_active_vstar_steven_semantic_override.inc`. Canonical route owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/active_vstar_steven_route_policy.inc Parent composition layer: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/part_issue_1067_arven_before_late_steven_override.inc Route specification: https://github.com/FlareZ123/pokemon-sims/issues/3203 C++ textual-include semantics: https://eel.is/c++draft/cpp.include
...
Route specifications: https://github.com/FlareZ123/pokemon-sims/issues/1745 https://github.com/FlareZ123/pokemon-sims/issues/1771 https://github.com/FlareZ123/pokemon-sims/issues/1772 https://github.com/FlareZ123/pokemon-sims/issues/2622 https://github.com/FlareZ123/pokemon-sims/issues/3653 https://github.com/FlareZ123/pokemon-sims/issues/3202 https://github.com/FlareZ123/pokemon-sims/issues/3203
...
Post-search composition owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/composition/post_014a_overrides.inc
C++ textual-include semantics: https://eel.is/c++draft/cpp.include
...
`cleanup-1786765850047` centralizes exact-print Forretress/Regidrago board-role predicates in `core/forretress/contract.inc` and aligns the internal pointer-to-index helper with the public `board_index_for_pointer()` naming. The next runtime step is to consume those predicates in `use_exploding_energy_for_setup()` before considering any retreat-ranking abstraction. Forretress contract: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/forretress/contract.inc Board owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/board_state_policy.inc
