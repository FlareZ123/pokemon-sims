...
- Legacy `name()` and `is_item()` compatibility cases are removed. Registered metadata is the sole display-name and Item-subtype owner; `is_ace_spec()` now delegates registered ACE SPEC ownership to `CardDefinition::ace_spec`, leaving only unmigrated Secret Box in the legacy fallback. Exact ACE SPEC print: https://api.pokemontcg.io/v2/cards/sv8-164
...
- `is_ace_spec()` now follows the same registered-first compatibility pattern. Brilliant Blender is read from exact-print registry metadata, while Secret Box remains the sole unmigrated legacy ACE SPEC fallback. Registry: https://github.com/FlareZ123/pokemon-sims/blob/main/src/cards/card_registry.hpp Brilliant Blender: https://api.pokemontcg.io/v2/cards/sv8-164
...
- Registered ACE SPEC ownership now uses `registered_is_ace_spec()` and `CardDefinition::ace_spec`; compatibility code no longer hard-codes Brilliant Blender as a second intrinsic owner. Exact print: https://api.pokemontcg.io/v2/cards/sv8-164
