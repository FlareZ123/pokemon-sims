...
- Completed: `src/trace_engine_v2/composition/post_014a_overrides.inc` now includes `src/trace_engine_v2/core/routes/oricorio_connector_policy.inc` directly at the proven post-search connector boundary. The former `src/trace_engine_v2/core/oricorio_connector_policy.inc` compatibility forwarder is retired. Oricorio / Vital Dance: https://api.pokemontcg.io/v2/cards/sm2-55 Regidrago VSTAR / Apex Dragon: https://api.pokemontcg.io/v2/cards/swsh12-136 Canonical owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/oricorio_connector_policy.inc
- The Oricorio cleanup preserves the existing route implementation, textual include order, DCI/UDP/AMR, connector domination, K0/K1 timing, route priority, lock admission, action ordering, and readiness behavior. C++ textual-include semantics: https://eel.is/c++draft/cpp.include
- Next safe route-owner follow-up remains the late-Steven/Burnet compatibility forwarder documented above; prove its live parent declaration boundary independently before changing it.
