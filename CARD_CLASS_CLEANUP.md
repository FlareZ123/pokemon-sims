...
`src/trace_engine_v2/core/board_state_policy.inc` now owns Active-first mutable/const pointer traversal, the shared `BoardIndex`/`OptionalBoardIndex`/`AttachmentDestinations` vocabulary, the `0 == Active` and `Bench index + 1` index mapping helper, and shared prior-turn evolution timing. `src/trace_engine_v2/core/forretress/contract.inc` consumes those types instead of redeclaring the mapping. Pineco evolution-candidate lookup remains delegated through the same board seam while Forest of Vitality's entry-turn exception stays at the Forretress route owner. This completes the shared board-index seam without changing DCI/UDP/AMR, K0/K1, connector, readiness, Ability, or evolution policy.

Next mechanical Forretress step: migrate Exploding Energy's remaining Active-versus-Bench source and Regidrago target scans to `find_board_pokemon_index_matching()`, then consider moving `pokemon_at_board_index()` into the same shared policy owner. Preserve Exploding Energy selection, attachment distribution, self-Knock-Out, promotion, route policy, and the established `0 == Active` / `Bench index + 1` mapping.
