...
`src/trace_engine_v2/composition/engine_body.inc` is the canonical Engine composition owner. `composition/opening_engine_overrides.inc` owns the early Supporter/VSTAR continuation. `composition/post_014a_overrides.inc` owns late-search composition. `composition/opening_legacy_stage.inc` now owns the historical `part_003.inc` through `part_005.inc` opening chain and its temporary aliases. `composition/middle_policy_stage.inc` sequences the named banked-Tapu and lock-removal substages. `composition/banked_tapu_policy_stage.inc` owns the banked-Tapu alias, search-item runtime, and route-policy boundary. `composition/lock_removal_policy_stage.inc` owns the shared lock-removal include and the `post_014a_overrides.inc` alias boundary. `composition/late_legacy_stage.inc` now owns the historical `part_014c.inc` through `part_016.inc` execution, registry, reporting chain and its cross-fragment aliases.
...
Next composition step: inventory the named middle substages for a smaller composition-only seam only when source-contract coverage can assert its exact textual boundary. Keep banked-Tapu route decisions in `core/routes/banked_tapu_retreat_policy.inc`, lock-removal decisions in their existing policy owners, and keep each alias setup attached to the include/teardown sequence it protects.
