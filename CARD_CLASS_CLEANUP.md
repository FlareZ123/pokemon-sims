...
## Cleanup wave 2026-08-14 Forretress composition consolidation

- `src/trace_engine_v2/core/forretress/composition.inc` is the canonical composition boundary for the existing Forretress runtime and Garbodor scenario modules. Canonical composition: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/forretress/composition.inc
- `src/trace_engine_v2/part_forretress_ex_combo.inc` remains only as the live namespace-scope compatibility boundary and now forwards to that single canonical composition owner. This preserves the proven include position while centralizing the module list under `core/forretress/`.
- The unreferenced `src/trace_engine_v2/core/forretress_combo_contract.inc` forwarding shim is retired. Contract ownership remains in `src/trace_engine_v2/core/forretress/contract.inc`: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/forretress/contract.inc
- This wave is composition-only. It changes no card resolution, DCI/UDP/AMR, connector priority, K0/K1 state, lock behavior, or setup policy. C++ textual-include semantics: https://eel.is/c++draft/cpp.include
