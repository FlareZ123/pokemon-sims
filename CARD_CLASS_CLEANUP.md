...
## Cleanup wave 2026-08-14 Ultra Ball metadata and forwarding checkpoint

- Enhancement #3517 is reclaimed after its previous branch and PR became stale: https://github.com/FlareZ123/pokemon-sims/issues/3517 https://github.com/FlareZ123/pokemon-sims/pull/3584
- `src/cards/trainers/ultra_ball.hpp` and `kRegisteredCardDefinitions` now own Ultra Ball's exact `sv1-196` identity, display name, Trainer kind, Item subtype, and printed two-other-card discard cost. Exact card data: https://api.pokemontcg.io/v2/cards/sv1-196
- Compatibility `name()` and `is_item()` now resolve Ultra Ball through the registry. The remaining legacy Item fallback is Earthen Vessel, whose separate migration remains tracked by https://github.com/FlareZ123/pokemon-sims/issues/3475
- Ultra Ball route admission, DCI/UDP discard valuation, discard selection, Pokémon target choice, K0/K1 transitions, search resolution, connector domination, and readiness policy remain at their existing Engine owners. The metadata migration does not authorize behavior changes. Architecture contract: https://github.com/FlareZ123/pokemon-sims/blob/main/CARD_CLASS_CLEANUP.md#card-module-contract
- The inactive `part_001.inc` payload mirror is retired so `core/card_classification.inc` remains the single live payload predicate owner. Existing source-anchor compatibility comments remain in the wrapper.
- The unreferenced `src/trace_engine_v2/core/oricorio_connector_policy.inc` forwarding shim is retired after the current composition and repository reference audit showed no live include dependency. The substantive owner remains `src/trace_engine_v2/core/routes/oricorio_connector_policy.inc`: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/oricorio_connector_policy.inc
- This wave must preserve strict Release compilation, representative `--simulate-this` action ordering, and the paired T2/T3 probability matrix. Any gameplay or probability drift requires a separately confirmed bug workflow.
