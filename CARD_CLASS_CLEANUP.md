...
`src/trace_engine_v2/core/board_state_policy.inc` owns the shared Active-first mutable/const board traversal, prior-turn evolution timing predicate, canonical `BoardIndex` / `OptionalBoardIndex` vocabulary, shared `BoardAttachmentDestinations` storage, and pointer-to-index conversion. The board-index mapping is named by `kActiveBoardIndex`, `board_index_for_bench_position()`, and `bench_position_from_board_index()`. Mutable/const pointer lookup reuses the same index-discovery plus index-lookup path, while `board_index_for_pointer()` provides one shared conversion for an already-selected board Pokémon. `src/trace_engine_v2/core/forretress/contract.inc` now consumes the shared attachment-destination storage through its card-facing `AttachmentDestinations` alias. This cleanup changes structure only; DCI/UDP/AMR, K0/K1, connector, readiness, Ability, and evolution policy remain at their prior owners.

Completed in cleanup branch `cleanup-1786758957236`: centralize Exploding Energy's board-index container type and add a reusable pointer-to-index seam before altering the runtime resolver. This keeps the next runtime edit small and mechanically reviewable.

Next mechanical Forretress step: replace `exploding_energy_regidrago_target_index()`'s Active/Bench pointer scan with `board_index_for_pointer(target_regi())`; replace the Exploding Energy source scan with `board_index_matching()`; route destination lookup through `board_pokemon_at_index()`; then remove the card-local `pokemon_at_board_index()` member after its final caller disappears. Preserve attachment distribution, self-Knock-Out, promotion, DCI/UDP/AMR, K0/K1, and route policy at the Forretress owner.
