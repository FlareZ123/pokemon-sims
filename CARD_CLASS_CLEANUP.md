...
`src/trace_engine_v2/composition/tapu_fss_overrides.inc` now owns the complete Tapu connector, late search-item wrapper, Pokémon Communication, and Forest Seal Stone target stack formerly embedded in `opening_engine_overrides.inc`. The extraction preserves the identical post-Steven textual boundary. All `bench_tapu_if_useful`, `needs_tapu_connector`, `in_play`, search-item, and `fss_target_after_search_started` aliases are established and released inside the named owner. Tapu Lele-GX: https://api.pokemontcg.io/v2/cards/sm2-60 Forest Seal Stone: https://api.pokemontcg.io/v2/cards/swsh12-156 Canonical owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/composition/tapu_fss_overrides.inc
