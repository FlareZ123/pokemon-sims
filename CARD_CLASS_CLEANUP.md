...
The free-slot connector now names its two route facts independently: whether Brilliant Blender is searchable and whether Steven should bank the free Blender package. Keep those facts local to the canonical route fragment so future cleanup can move the whole continuation without reconstructing connector-domination state from a single compound expression.
...
The next numbered-fragment cleanup is staged explicitly: retarget the single `composition/opening_engine_overrides.inc` include from `../part_013.inc` to `../core/supporter_legacy_runtime.inc`, run raw-source and documentation-anchor checks, then delete `part_013.inc` only after strict compilation and trace CI prove the exact boundary. This sequence preserves the runtime blob unchanged while removing the compatibility layer in a later cleanup PR.
