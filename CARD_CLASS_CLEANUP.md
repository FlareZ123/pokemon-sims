...
- `src/trace_engine_v2/core/routes/battle_compressor_vs_seeker_policy.inc` is the canonical Battle Compressor / VS Seeker route owner and is now included directly by `composition/engine_body.inc`; the historical forwarding include has been retired. Battle Compressor: https://api.pokemontcg.io/v2/cards/xy4-92 VS Seeker: https://api.pokemontcg.io/v2/cards/xy4-109 Advanced procedure: https://github.com/FlareZ123/pokemon-sims/blob/main/EN_advanced_manual-2025-transcription-structured.md
...
The Battle Compressor / VS Seeker route now follows the same direct canonical composition pattern: `src/trace_engine_v2/composition/engine_body.inc` includes `src/trace_engine_v2/core/routes/battle_compressor_vs_seeker_policy.inc` at the unchanged pre-`part_014c.inc` boundary. The forwarding-only `src/trace_engine_v2/core/battle_compressor_vs_seeker_policy.inc` layer has been deleted after repository-wide direct-path references were checked. This changes ownership visibility only; route admission, DCI/JIT policy, card effects, and action order remain in the canonical route owner. Canonical owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/battle_compressor_vs_seeker_policy.inc C++ textual-include semantics: https://eel.is/c++draft/cpp.include
...
14. Treat the Battle Compressor / VS Seeker forwarding-layer migration as complete. New composition or source-contract references must target `core/routes/battle_compressor_vs_seeker_policy.inc` directly; do not recreate the historical root forwarding include.
