...
`src/trace_engine_v2/core/deck_knowledge.inc` now gives resolved zone visibility typed Engine-local value owners. `PublicZoneCopyCounts` owns K0 public hand/discard/attached counts, while `KnownSearchCopyCounts` owns K1 hand/known-deck counts. `KnowledgeCopyPolicy` remains the arithmetic primitive. `public_unattached_zone_copy_counts()` intentionally skips attached-board scanning for callers that only need hand plus discard, and `public_zone_copy_counts()` adds attached public copies. `known_search_copy_counts()` obtains deck membership only through the K1-gated `known_deck_copy_count()`. Canonical owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/deck_knowledge.inc K0/K1 visibility rules: https://github.com/FlareZ123/pokemon-sims/blob/main/docs/POLICY_DECISIONS.md#knowledge-states

Next catalog/knowledge step: reuse these typed count bundles when route-local code repeats the exact same already-resolved visibility boundary. Migrate legacy name and intrinsic metadata rows only after explicit `CardDefinition` registration and coverage. Keep hidden-zone visibility, Prize deduction, search timing, target preference, DCI/UDP/AMR, and route admission at strategy owners.
