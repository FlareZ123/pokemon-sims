...
- `src/trace_engine_v2/core/routes/search_connector_helpers.inc` owns `need_regi`, `need_vstar`, payload-outlet checks, one shared K1 deck-scan primitive, and the complete K1 fallback selectors for Mysterious Treasure, Quick Ball, and Ultra Ball. Each selector retains its own printed target order while the repeated physical deck scan is centralized. `part_008b.inc` composes this owner at the same Engine member boundary, while `part_009a.inc` starts with the remaining Mysterious Treasure route body. DCI/UDP/AMR, K0/K1 visibility, and route-specific target choice remain Engine strategy concerns. Mysterious Treasure: https://api.pokemontcg.io/v2/cards/sm6-113 Quick Ball: https://api.pokemontcg.io/v2/cards/swsh1-179 Ultra Ball: https://api.pokemontcg.io/v2/cards/swsh12pt5-146 Earthen Vessel: https://api.pokemontcg.io/v2/cards/sv4-163 Advanced procedure: https://github.com/FlareZ123/pokemon-sims/blob/main/EN_advanced_manual-2025-transcription-structured.md
- `src/trace_engine_v2/core/routes/mysterious_treasure_prerequisite_policy.inc` owns the complete Burnet-to-Treasure VSTAR cost projection, Burnet/Treasure route-availability check, and K0/K1 legal-target existence query that previously opened `part_009a.inc`. The named owner is composed at the identical Engine member boundary before ordered target policy and action execution. Mysterious Treasure: https://api.pokemontcg.io/v2/cards/sm6-113 Professor Burnet: https://api.pokemontcg.io/v2/cards/swsh12tg-TG26 Regidrago VSTAR: https://api.pokemontcg.io/v2/cards/swsh12-136 C++ textual-include semantics: https://eel.is/c++draft/cpp.include
- `src/trace_engine_v2/core/mysterious_treasure_target_policy.inc` now owns the established ordered Mysterious Treasure target priority through `MysteriousTreasureTargetPolicy`. The remaining `part_009a.inc` route delegates pure physical payload-zone membership to `PayloadZonePolicy`, while DCI/JIT admission, K0/K1 target visibility, connector domination, and action execution remain Engine-owned. Mysterious Treasure: https://api.pokemontcg.io/v2/cards/sm6-113 Payload owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/payload_hand_policy.inc
...
The complete search-connector preparation and post-search fallback helpers have one named owner at `src/trace_engine_v2/core/routes/search_connector_helpers.inc`. The owner now centralizes the repeated K1 physical deck scan while preserving separate ordered target sets for Mysterious Treasure, Quick Ball, and Ultra Ball. `part_008b.inc` includes that owner directly after the Heavy Ball policy. The next numbered boundary begins with the Mysterious Treasure route in `part_009a.inc`. C++ textual-include semantics: https://eel.is/c++draft/cpp.include Quick Ball: https://api.pokemontcg.io/v2/cards/swsh1-179 Ultra Ball: https://api.pokemontcg.io/v2/cards/swsh12pt5-146

The Mysterious Treasure prerequisite and legal-target query members now live in `src/trace_engine_v2/core/routes/mysterious_treasure_prerequisite_policy.inc`, composed directly at the start of `part_009a.inc`. This preserves their exact member order before `MysteriousTreasureTargetPolicy` and `play_mysterious_treasure`, while shrinking the numbered fragment to the remaining route continuation. Mysterious Treasure: https://api.pokemontcg.io/v2/cards/sm6-113 C++ textual-include semantics: https://eel.is/c++draft/cpp.include
...
- Mysterious Treasure prerequisite and legal-target queries: `src/trace_engine_v2/core/routes/mysterious_treasure_prerequisite_policy.inc`.
- Mysterious Treasure ordered target priority: `src/trace_engine_v2/core/mysterious_treasure_target_policy.inc` via `MysteriousTreasureTargetPolicy`.
...
7. Keep Forretress reusable scenario and runtime ownership under `core/forretress/`, with `core/forretress/package.inc` as the namespace-scope composition owner and `core/forretress/scenario_contract.inc` as the registry declaration owner. Migrate the remaining `part_014c.inc` consumer to the canonical package before deleting `part_forretress_ex_combo.inc`.
...
15. Keep `core/routes/mysterious_treasure_prerequisite_policy.inc` and `core/mysterious_treasure_target_policy.inc` as the named prerequisite and ordered-target owners. The remaining `play_mysterious_treasure` continuation may leave `part_009a.inc` only when its complete textual member boundary can migrate atomically without changing wrapper alias lifetime or declaration order. C++ textual-include semantics: https://eel.is/c++draft/cpp.include Mysterious Treasure: https://api.pokemontcg.io/v2/cards/sm6-113
