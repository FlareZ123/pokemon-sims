...
- `src/trace_engine_v2/core/forretress/scenario_registry.inc` owns the Forretress-side public scenario registry contract plus shared and Garbodor scenario extensions. `core/forretress/package.inc` composes runtime first, then this registry owner, preserving the established namespace-scope order. Canonical registry owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/forretress/scenario_registry.inc
...
The namespace-scope Forretress composition remains rooted at `src/trace_engine_v2/core/forretress/package.inc`. The package now composes `runtime.inc` followed by the named `scenario_registry.inc`; that registry owner holds the public scenario aliases and forward declarations, then composes `scenario_extension.inc` and `garbodor_scenario_extension.inc` in the same established order. The historical `src/trace_engine_v2/part_forretress_ex_combo.inc` path remains a thin compatibility include until repository-wide and source-contract consumers are proven migrated. Canonical package: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/forretress/package.inc Canonical registry owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/forretress/scenario_registry.inc Garbodor: https://api.pokemontcg.io/v2/cards/xy9-57 Boost Shake: https://api.pokemontcg.io/v2/cards/swsh7-142 C++ textual-include semantics: https://eel.is/c++draft/cpp.include
...
- Forretress namespace-scope scenario registry and extensions: `src/trace_engine_v2/core/forretress/scenario_registry.inc`.
...
7. Keep Forretress runtime composition in `core/forretress/package.inc` and scenario-registry composition in `core/forretress/scenario_registry.inc`. Add future namespace-scope scenario extensions through the registry owner so public declarations and extension ordering stay centralized. Remove `part_forretress_ex_combo.inc` only after repository-wide and source-contract consumers are proven migrated.
