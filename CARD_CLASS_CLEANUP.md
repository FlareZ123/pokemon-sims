...
`src/trace_engine_v2/composition/engine_body.inc` is the canonical Engine composition owner. `composition/opening_engine_overrides.inc` owns the early Supporter/VSTAR continuation. `composition/post_014a_overrides.inc` owns late-search composition. `composition/opening_legacy_stage.inc` owns the historical `part_003.inc` opening edge and delegates its continuation to `composition/opening_state_completion_stage.inc`, which owns the Garbodor ability alias, `part_004.inc` / `part_005.inc`, and the `might_be_unseen` teardown. `composition/engine_body.inc` now sequences `composition/banked_tapu_policy_stage.inc` and `composition/lock_removal_policy_stage.inc` directly at the middle boundary, then sequences `composition/late_execution_stage.inc` and `composition/late_registry_reporting_stage.inc` directly at the late boundary. The banked-Tapu substage owns its temporary alias, search-item runtime, and route-policy boundary; the lock-removal substage owns the shared lock-removal include and `post_014a_overrides.inc` alias boundary; the two late substages own the `part_014c.inc` execution aliases and the `part_015.inc` / `part_016.inc` registry-reporting aliases respectively. Canonical composition owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/composition/engine_body.inc
...
Completed in `cleanup-1786786368078`: retired the forwarding-only `composition/middle_policy_stage.inc` and `composition/late_legacy_stage.inc` sequencers after retargeting `composition/engine_body.inc` at the identical textual boundaries. The canonical owner now composes the banked-Tapu, lock-removal, late-execution, and late-registry/reporting substages directly. Macro setup/teardown and gameplay logic remain inside their existing substages, so route admission, DCI/UDP/AMR, K0/K1, Supporter contention, connector domination, and card-resolution behavior are unchanged. C++ textual include semantics: https://eel.is/c++draft/cpp.include Canonical composition owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/composition/engine_body.inc

Next composition step: inventory the remaining named composition stages for another forwarding-only seam whose parent can be retargeted at the identical textual boundary. Keep gameplay route decisions in their current policy owners, keep each alias setup attached to the include/teardown sequence it protects, and retain `opening_legacy_stage.inc` while it still owns live alias setup rather than forwarding only.
...
Completed in `cleanup-1786787925047`: migrated the Active-VSTAR Steven hand-presence predicate and both issue-1447 Earthen Vessel hand-presence predicates from route-local `std::any_of(..., is_payload)` scans to the canonical `payload_zone_contains()` seam. These callers require only Boolean membership, so physical selection order, strategic preference order, DCI/JIT admission, K0/K1 timing, discard timing, and route decisions remain unchanged. Shared owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/payload_hand_policy.inc Regidrago VSTAR / Apex Dragon: https://api.pokemontcg.io/v2/cards/swsh12-136 Decision policy: https://github.com/FlareZ123/pokemon-sims/blob/main/docs/POLICY_DECISIONS.md#dcijit-treatment

Next payload step: continue inventorying ad hoc Dragon-payload scans. Migrate pure Boolean membership callers to `PayloadZonePolicy::contains()`, first-physical-match callers only to `PayloadZonePolicy::first()`, and strategic-priority callers only to the matching `PayloadPreferencePolicy` seam. Preserve physical order whenever it is observable and keep DCI/JIT predicates, K0/K1 timing, and discard timing at their strategy owners.
