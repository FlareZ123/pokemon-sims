...
The issue-1516/2164 Quick Ball, Tapu Lele-GX, Crispin route family has a canonical semantic owner at `src/trace_engine_v2/core/routes/quick_ball_tapu_crispin_policy.inc`. The historical `part_issue_1516_quick_ball_tapu_crispin_override.inc` compatibility seam is retired after `src/trace_engine_v2/composition/post_014a_overrides.inc` was rewired directly to the canonical owner under the same `play_quick_ball -> play_quick_ball_issue1595_original` macro lifetime. Route admission, copied-Engine projection, K1 checks, lock checks, trace text, and direct source URLs remain together under the canonical route owner. Quick Ball: https://api.pokemontcg.io/v2/cards/swsh1-179 Tapu Lele-GX: https://api.pokemontcg.io/v2/cards/sm2-60 Crispin: https://api.pokemontcg.io/v2/cards/sv7-133 Canonical owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/quick_ball_tapu_crispin_policy.inc Composition owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/composition/post_014a_overrides.inc

Next composition step: inspect another root `part_*` seam only when its complete macro lifetime or function body can move intact to an existing semantic owner. Keep tooling-only compatibility paths minimal, preserve declaration order and route semantics, and retain direct source URLs beside rule-sensitive logic.
