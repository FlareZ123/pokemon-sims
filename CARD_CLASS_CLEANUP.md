...
- `PayloadPreferencePolicy::any_preferred_with_positive_count()` owns membership-only checks over count-backed zones while preserving the same canonical five-Dragon inventory.
...
The Quick Ball/Tapu/Crispin known-Prize route now delegates its payload membership question to `PayloadPreferencePolicy::any_preferred_with_positive_count()`. This removes a route-local five-card array and keeps the accepted Dragon inventory synchronized with the selector used by other payload policies. Route owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/quick_ball_tapu_crispin_policy.inc Canonical payload owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/payload_hand_policy.inc Regidrago VSTAR / Apex Dragon: https://api.pokemontcg.io/v2/cards/swsh12-136
