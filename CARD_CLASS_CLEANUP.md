...
## Cleanup wave 2026-08-14 root card source-contract cleanup

- `src/trace_engine_v2/composition/engine_body.inc` directly owns normal composition of `core/card_catalog.inc` and `core/card_classification.inc`. The root `part_001.inc` compatibility shim is retired, while `part_000.inc` remains a minimal seven-line compatibility seam because `tests/generate_unified_tests.py` still reads that path for source-header discovery. Canonical catalog: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/card_catalog.inc Canonical classification: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/card_classification.inc Unified test generator: https://github.com/FlareZ123/pokemon-sims/blob/main/tests/generate_unified_tests.py
- This removes legacy source-anchor comments and the inactive `#if 0` classification mirror while preserving card behavior, DCI/UDP/AMR, connector domination, K0/K1 timing, route ordering, and printed resolution. C++ textual-include semantics: https://eel.is/c++draft/cpp.include
- Retire `part_000.inc` only after unified-test header discovery is migrated to the canonical owner. Validate strict compilation, full regression and sanitizer suites, permanent `--simulate-this` audits, and the paired T2/T3 matrix before merge.
