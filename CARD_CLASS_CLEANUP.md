...
9. Continue moving complete root `part_*` route bodies into `core/routes/` one owner at a time. Leave a forwarding seam at the historical path until source-contract references are migrated, and preserve the exact Engine member boundary plus all rule and issue provenance during each move.
