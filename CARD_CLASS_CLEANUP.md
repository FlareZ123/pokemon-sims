...
The namespace-scope Forretress composition is canonically owned by `src/trace_engine_v2/core/forretress/package.inc`. Repository-wide reference checks showed the former root `src/trace_engine_v2/part_forretress_ex_combo.inc` compatibility include had no remaining consumers, so that obsolete forwarding seam is retired. The package keeps the reusable runtime, public scenario forward declarations, shared scenario extension, and Garbodor scenario family at the same verified namespace boundary while preserving their textual order. Canonical package: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/forretress/package.inc C++ textual-include semantics: https://eel.is/c++draft/cpp.include

The Steven/Brilliant Blender semantic route owners are `src/trace_engine_v2/core/routes/k0_steven_blender_semantic_policy.inc` and `src/trace_engine_v2/core/routes/steven_blender_semantic_policy.inc`. Repository-wide reference checks showed the former issue-3221 and issue-3222 root compatibility includes had no remaining consumers, so both obsolete root forwarding seams are retired. Brilliant Blender: https://api.pokemontcg.io/v2/cards/sv8-164 Steven's Resolve: https://api.pokemontcg.io/v2/cards/sm7-145 Regidrago VSTAR: https://api.pokemontcg.io/v2/cards/swsh12-136 Advanced procedure: https://github.com/FlareZ123/pokemon-sims/blob/main/EN_advanced_manual-2025-transcription-structured.md
...
7. Keep Forretress reusable scenario and runtime ownership under `core/forretress/`, with `core/forretress/package.inc` as the namespace-scope composition owner. The former root Forretress forwarding seam is retired and should not be recreated.
8. Collapse the `part_007.inc` / `part_008a.inc` continuation only when the full `evolve_best_regi` body can move as one textual unit with identical declaration and macro context; the current split crosses a live function body and requires atomic ownership migration.
