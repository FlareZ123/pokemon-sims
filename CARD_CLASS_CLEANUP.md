...
## Cleanup wave 2026-08-14 issue-3202 route ownership

- `src/trace_engine_v2/core/routes/issue_3202_steven_vstar_vessel_policy.inc` is now the single implementation owner for the confirmed #3202 Steven/VSTAR/Vessel route policy. The move preserves every existing Steven's Resolve, Earthen Vessel, Regidrago VSTAR, advanced-manual, repository-policy, and confirmed-issue source URL. Canonical owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/issue_3202_steven_vstar_vessel_policy.inc
- `src/trace_engine_v2/part_issue_3202_steven_vstar_vessel_semantic_override.inc` remains a thin compatibility forwarder because the live composition parent still includes the historical path. This follows the route-policy plan's temporary-forwarder rule and preserves declaration and macro order. C++ textual include semantics: https://eel.is/c++draft/cpp.include
- Next safe follow-up: prove the consuming parent boundary in CI, include the canonical owner directly from that parent, then retire the historical forwarder. Keep that follow-up mechanical and do not mix it with any pending gameplay correction, including https://github.com/FlareZ123/pokemon-sims/issues/3601
