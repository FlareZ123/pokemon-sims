...
- `src/trace_engine_v2/composition/engine_body.inc` directly composes `core/card_catalog.inc` and `core/card_classification.inc`; the root `part_000.inc` and `part_001.inc` compatibility shims are retired after repository-reference review. Canonical catalog: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/card_catalog.inc Canonical classification: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/card_classification.inc
- The cleanup removes forwarding and source-mirror scaffolding while preserving card metadata, classification behavior, DCI/UDP/AMR, connector domination, K0/K1 state, route ordering, and printed resolution. Engine composition: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/composition/engine_body.inc C++ textual-include semantics: https://eel.is/c++draft/cpp.include
- Keep future root `part_*.inc` retirement limited to paths with no live parent include or source-contract dependency, then validate strict compilation, the regression suites, permanent `--simulate-this` audits, and the paired T2/T3 matrix before merge.
