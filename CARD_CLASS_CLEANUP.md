...
`src/trace_engine_v2/core/board_state_policy.inc` owns the shared Active-first mutable/const board traversal, canonical `BoardIndex` / `OptionalBoardIndex` types, the `0 == Active` and `Bench index + 1` coordinate mapping through `board_index_matching()`, and the shared prior-turn evolution timing predicate. `src/trace_engine_v2/core/forretress/contract.inc` retains only the Forretress-specific `AttachmentDestinations` alias. `src/trace_engine_v2/core/forretress/runtime.inc` already delegates Pineco evolution-candidate lookup through the shared pointer traversal while keeping Forest of Vitality's entry-turn exception at the Forretress route owner. These steps centralize board traversal mechanics without changing DCI/UDP/AMR, K0/K1, connector, readiness, Ability, or evolution policy.

Next mechanical Forretress step: migrate `exploding_energy_regidrago_target_index()` and Exploding Energy's Forretress source lookup in `core/forretress/runtime.inc` to `board_index_matching()`, then remove those raw Active-versus-Bench index loops. Keep Exploding Energy selection, attachment distribution, self-Knock-Out, promotion, and route policy at their current owners.
