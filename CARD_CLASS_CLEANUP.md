...
- `src/trace_engine_v2/core/tate/package.inc` owns the established discard-provenance, Tate attachment, and Tate action override order. `core/tate/discard_recovery_provenance.inc`, `core/tate/attachment_policy.inc`, and `core/tate/action_policy.inc` are now the live named lower-level owners; the historical root bodies remain source-contract mirrors while references migrate. Canonical package: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/tate/package.inc Canonical action owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/tate/action_policy.inc
...
The Tate package now composes discard/recovery provenance, proactive attachment, and action policy from `core/tate/discard_recovery_provenance.inc`, `core/tate/attachment_policy.inc`, and `core/tate/action_policy.inc` at the same member positions. The historical root bodies remain source-contract mirrors while references migrate, but composition no longer depends on the root Tate action fragment. Canonical package: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/tate/package.inc Canonical action owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/tate/action_policy.inc C++ textual-include semantics: https://eel.is/c++draft/cpp.include
...
- Tate/provenance composition: `src/trace_engine_v2/core/tate/package.inc`; discard/recovery provenance, proactive attachment, and Tate action policy now compose from named lower-level owners under `core/tate/`. The root Tate action body remains a source-contract mirror only.
...
8. Keep `core/tate/package.inc` as the sole Tate/provenance composition include and `core/tate/action_policy.inc` as the canonical action owner. Migrate source-contract anchors away from `part_tate_blender_tate_override.inc`, then retire the historical Tate root mirrors only when no audit or traceability reference still depends on them. C++ textual-include semantics: https://eel.is/c++draft/cpp.include
