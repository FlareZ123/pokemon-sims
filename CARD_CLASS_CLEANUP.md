...
Next shared-policy step: remove only forwarding-only Engine wrappers proven unused by repository-wide search, then route duplicate Garbodor present/removed plus Rule Box composition through `ability_available_from_lock_state()` where all four state facts have the same semantics. Keep `garbotoxin_locked()`, `rule_box_ability_available()`, and `ability_available()` available for callers that already hold narrower or resolved facts. Preserve separate scenario identity, activation-turn, lock-removal, and Rule Box queries where callers need one fact without the others.
