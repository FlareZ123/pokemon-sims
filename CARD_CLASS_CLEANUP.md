...
The Tate package now composes discard/recovery provenance, proactive attachment, and Tate action policy from `core/tate/discard_recovery_provenance.inc`, `core/tate/attachment_policy.inc`, and `core/tate/action_policy.inc` at the same member positions. The three historical root bodies are retained only as source-contract mirrors while references migrate. Canonical package: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/tate/package.inc C++ textual-include semantics: https://eel.is/c++draft/cpp.include
...
8. Keep the Tate/provenance package as the sole composition include. All three Tate bodies now have named owners under `core/tate/`; migrate source-contract and traceability anchors to those canonical paths before retiring the historical root mirrors.
