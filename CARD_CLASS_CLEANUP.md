...
- `src/trace_engine_v2/core/tate/package.inc` owns the established discard-provenance, Tate attachment, and Tate action override order. `core/tate/discard_recovery_provenance.inc` and `core/tate/attachment_policy.inc` own the first two bodies, and `core/tate/action_policy.inc` now owns the named package seam for the action body. The historical root action file remains the live implementation behind that seam until source-contract anchors can migrate safely: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/tate/package.inc
...
The Tate package now composes discard/recovery provenance and proactive attachment from `core/tate/discard_recovery_provenance.inc` and `core/tate/attachment_policy.inc`, then enters `core/tate/action_policy.inc` at the same member position. The action policy is currently a behavior-neutral named seam over `part_tate_blender_tate_override.inc`; the next migration is to move that implementation body behind the seam after source-contract anchors are updated. Canonical package: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/tate/package.inc Canonical action seam: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/tate/action_policy.inc C++ textual-include semantics: https://eel.is/c++draft/cpp.include
...
- Tate/provenance composition: `src/trace_engine_v2/core/tate/package.inc`; discard/recovery provenance and proactive attachment compose from named lower-level owners under `core/tate/`, and `core/tate/action_policy.inc` is now the named action seam over the historical root implementation.
...
8. Keep the Tate/provenance package as the sole composition include. `core/tate/action_policy.inc` now provides the named action seam at the historical member boundary; next migrate source-contract anchors and then move the root action implementation into that canonical owner without changing declaration order or route policy.
