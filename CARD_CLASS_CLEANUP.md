...
1. Keep `composition/steven_blender_overrides.inc` and `core/routes/steven/package.inc` as the canonical Steven organization boundaries. Continue deleting forwarding paths only after both repository reference checks and CI compile checks prove the historical path is unused.
...
7. Keep Forretress reusable scenario and runtime ownership under `core/forretress/`, with `core/forretress/package.inc` as the namespace-scope composition owner. Migrate the live `part_014c.inc` include directly to that canonical package at the same namespace boundary before deleting `part_forretress_ex_combo.inc`.
