...
2026-08-21 compatibility-seam checkpoint: `src/trace_engine_v2/core/regidrago_line_helpers.inc` now uses one `#pragma once` boundary plus the canonical-classification ordering assertion. It contains no duplicate classifier body and does not reopen any retired implementation. Keep the seam until `composition/post_014a_overrides.inc` can consume the catalog-owned classifier without a historical include site. Canonical classifier: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/card_catalog.inc Regidrago V: https://api.pokemontcg.io/v2/cards/swsh12-135 Regidrago VSTAR: https://api.pokemontcg.io/v2/cards/swsh12-136

2026-08-21 Legacy Star provenance checkpoint: `LegacyStarProjectionProvenanceState` now owns restoration of its projected removed-card marker into `discarded_this_turn`. Engine keeps route timing and speculative-plan orchestration, while the small state object owns marker lifetime and reset. Preserve the direct Legacy Star and Pokémon Communication sources when this helper moves again. Legacy Star: https://api.pokemontcg.io/v2/cards/swsh12-136 Pokémon Communication: https://api.pokemontcg.io/v2/cards/sm9-152
