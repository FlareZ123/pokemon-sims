...
`src/trace_engine_v2/composition/engine_body.inc` is the canonical Engine composition owner. `composition/opening_engine_overrides.inc` owns the early Supporter/VSTAR continuation. `composition/post_014a_overrides.inc` owns late-search composition. `composition/opening_legacy_stage.inc` now owns the historical `part_003.inc` through `part_005.inc` opening chain and its temporary aliases. `composition/middle_policy_stage.inc` owns the banked-Tapu replacement and adjacent forest/Field Blower lock-removal alias block at their established member boundaries. `composition/late_legacy_stage.inc` now owns the historical `part_014c.inc` through `part_016.inc` execution, registry, reporting chain and its cross-fragment aliases.
...
Completed in `cleanup-1786777124595`: extracted the middle banked-Tapu boundary and its adjacent forest/Field Blower lock-removal alias block from `composition/engine_body.inc` into `composition/middle_policy_stage.inc`. The stage retains the exact `#define` / `#include` / `#undef` order, local entry/exit alias assertions, and established boundaries around `opening_engine_overrides.inc`, `turn_action_policy_runtime.inc`, `core/routes/banked_tapu_retreat_policy.inc`, `core/forest_field_blower_policy.inc`, and `post_014a_overrides.inc`. Route policy and gameplay behavior remain at their prior owners. Canonical stage: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/composition/middle_policy_stage.inc C++ textual-include semantics: https://eel.is/c++draft/cpp.include

Next composition step: inventory `opening_legacy_stage.inc`, `middle_policy_stage.inc`, and `late_legacy_stage.inc` for composition-only compatibility forwarders whose parent can be retargeted at the identical textual boundary. Retire only forwarders with exact source-contract coverage, and keep route admission/projection/decision policy in its existing `core/routes/` owner.
