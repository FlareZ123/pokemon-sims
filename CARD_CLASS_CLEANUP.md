...
The issue-1673 Secret Box deadline fallback now delegates first held-Dragon selection to `first_payload_card_in_zone(state_.hand)`, preserving the historical physical hand order. The active-VSTAR Steven/Crispin/Treasure route now delegates held-payload membership to `payload_zone_contains(state_.hand)` and known-deck Dragon-or-Psychic target cardinality to `PayloadZonePolicy::count_matching()`. These are query-only migrations; route admission, K1/JIT timing, cost planning, and target strategy remain at their existing owners. Secret Box route: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/issue_1673_secret_box_payload_deadline_policy.inc Active-VSTAR Steven route: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/active_vstar_steven_route_policy.inc Canonical query owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/payload_hand_policy.inc
