...
Next composition step: audit the three new Forest Seal Stone compatibility includes and retire each root path only after repository tooling and source-contract consumers are confirmed to use the canonical `core/routes/` owner. For other remaining root `part_*` seams, move only a live complete macro lifetime or function body at the identical textual boundary. Preserve declaration order, route semantics, direct source URLs, and existing helper reuse. Do not recreate forwarding-only sequencers; compatibility includes are temporary migration seams, not new semantic owners.
