...
- `src/trace_engine_v2/core/routes/issue_1016_legacy_star_quick_ball_policy.inc` owns the Legacy Star/Quick Ball connector and inertness projections through `LegacyStarQuickBallPolicy`. `src/trace_engine_v2/core/routes/legacy_star/quick_ball_projection_policy.inc` owns only the temporary hide/restore bookkeeping for the wrapped route view through `LegacyStarQuickBallProjectionPolicy`; Engine retains DCI/JIT state reads, connector admission, and the physical Legacy Star action. Quick Ball: https://api.pokemontcg.io/v2/cards/swsh1-179 Regidrago VSTAR / Legacy Star: https://api.pokemontcg.io/v2/cards/swsh12-136
...
- Legacy Star/Quick Ball decision projection: `src/trace_engine_v2/core/routes/issue_1016_legacy_star_quick_ball_policy.inc` via `LegacyStarQuickBallPolicy`; temporary route-view Quick Ball bookkeeping is centralized in `src/trace_engine_v2/core/routes/legacy_star/quick_ball_projection_policy.inc` via `LegacyStarQuickBallProjectionPolicy`.
...
16. Keep `LegacyStarQuickBallProjectionPolicy` limited to deterministic hide/restore bookkeeping for the issue-1016 wrapped route view. A future cleanup may remove the temporary hand mutation only after the legacy Legacy Star resolver can accept an explicit projection view without changing route order, hidden-information timing, DCI/JIT admission, or physical hand semantics. Quick Ball: https://api.pokemontcg.io/v2/cards/swsh1-179 Regidrago VSTAR / Legacy Star: https://api.pokemontcg.io/v2/cards/swsh12-136
