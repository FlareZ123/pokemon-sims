...
- `src/trace_engine_v2/core/simulation_runtime.inc` is now a thin ordered composition owner for runtime support. `core/runtime/state.inc` owns `DciProfile`, lock/tool/stadium enums, `Scenario`, `Pokemon`, and `State`; `core/runtime/metrics.inc` owns `TrialOutcome` and `Aggregate`; `core/runtime/trace_log.inc` owns `TraceLog` and fixed-point trace suppression. Preserve state -> metrics -> trace declaration order because later Engine fragments depend on those complete types. C++ textual-include semantics: https://eel.is/c++draft/cpp.include
...
- Runtime state model: `src/trace_engine_v2/core/runtime/state.inc`; keep strategic policy out of these data declarations.
- Runtime trial/aggregate metrics: `src/trace_engine_v2/core/runtime/metrics.inc`; keep mutation and route logic in Engine owners.
- Runtime trace ownership: `src/trace_engine_v2/core/runtime/trace_log.inc`; fixed-point fingerprinting remains a trace concern and consumes the already-defined `State` model.
...
16. Keep `core/simulation_runtime.inc` as an ordered composition file only. Future runtime cleanup should extend `core/runtime/state.inc`, `core/runtime/metrics.inc`, or `core/runtime/trace_log.inc` by responsibility instead of reintroducing mixed declarations into the root fragment. Before moving any additional runtime helper, prove its declaration-order dependencies and preserve the current state -> metrics -> trace sequence. C++ textual-include semantics: https://eel.is/c++draft/cpp.include
