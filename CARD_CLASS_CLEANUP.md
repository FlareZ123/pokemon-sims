...
- `src/trace_engine_v2/core/routes/mysterious_treasure/target_policy.inc` owns Mysterious Treasure strategic target priority. `src/trace_engine_v2/core/mysterious_treasure_target_policy.inc` is now a compatibility include for the live `part_009a.inc` boundary. Mysterious Treasure: https://api.pokemontcg.io/v2/cards/sm6-113
- `src/trace_engine_v2/core/routes/connectors/tapu_policy.inc` owns copy-aware Tapu Lele-GX connector admission and its adjacent historical connector aliases. `src/trace_engine_v2/core/tapu_connector_policy.inc` is now a compatibility include for the opening composition boundary. Tapu Lele-GX: https://api.pokemontcg.io/v2/cards/sm2-60
...
2026-08-21 route-package checkpoint: Mysterious Treasure target ordering and Tapu Lele-GX connector policy now live under `core/routes/`. Their old `core/*.inc` paths contain compatibility includes only, preserving the established textual composition boundaries until the numbered and opening consumers are migrated directly. Keep the canonical package as the sole substantive owner and delete each compatibility include as soon as its final consumer points at the package.
...
Mysterious Treasure keeps strategic Dragon/Psychic target preference in `core/routes/mysterious_treasure/target_policy.inc`; post-search K1 fallback legality and order stay in `core/routes/search_connector_helpers.inc`. The old `core/mysterious_treasure_target_policy.inc` path is compatibility-only. Mysterious Treasure: https://api.pokemontcg.io/v2/cards/sm6-113
...
- Tapu Lele-GX copy-aware connector admission: `src/trace_engine_v2/core/routes/connectors/tapu_policy.inc`.
...
2. Point `part_009a.inc` directly at `core/routes/mysterious_treasure/target_policy.inc`, then delete `core/mysterious_treasure_target_policy.inc`. Preserve target order and the direct Mysterious Treasure card-data citation.
3. Point the opening connector composition directly at `core/routes/connectors/tapu_policy.inc`, then delete `core/tapu_connector_policy.inc`. Preserve the existing alias lifetime around its consumer.
