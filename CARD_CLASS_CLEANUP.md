...
Completed in `cleanup-1786762529275`: `composition/opening_engine_overrides.inc` now composes `core/routes/steven_vstar_vessel_route_policy.inc` directly at the exact issue-3202 member and macro boundary, and the root `part_issue_3202_steven_vstar_vessel_semantic_override.inc` compatibility forwarder is retired. This is a structural include-path cleanup only; route admission, DCI/UDP/AMR, Supporter contention, connector domination, card legality, and trace behavior remain owned by the same canonical route policy. Canonical route: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/steven_vstar_vessel_route_policy.inc Steven's Resolve: https://api.pokemontcg.io/v2/cards/sm7-145

Next mechanical Steven step: inspect `part_issue_3203_active_vstar_steven_semantic_override.inc` and `part_issue_1191_gladion_steven_override.inc` as the next composition-only forwarder candidates. Retire either only after its live parent is retargeted to the canonical `core/routes/` owner at the identical member and macro boundary, and preserve any adjacent non-Steven continuation include. Preserve route admission, DCI/UDP/AMR, Supporter contention, connector domination, and all direct card/rule URLs while doing those moves. Steven's Resolve: https://api.pokemontcg.io/v2/cards/sm7-145 Advanced rules: https://github.com/FlareZ123/pokemon-sims/blob/main/EN_advanced_manual-2025-transcription-structured.md Decision priorities: https://github.com/FlareZ123/pokemon-sims/blob/main/docs/POLICY_DECISIONS.md#decision-priorities
