...
- `src/trace_engine_v2/core/tate/package.inc` owns the established discard-provenance, Tate attachment, and Tate action override order. `core/tate/discard_recovery_provenance.inc` and `core/tate/attachment_policy.inc` own the first two lower-level bodies, and `core/tate/action_policy.inc` is the named composition seam for the Tate action body. The historical root action body remains the rule-sensitive implementation and source-contract anchor until those references migrate: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/tate/package.inc https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/tate/action_policy.inc
...
The Tate package now composes discard/recovery provenance, proactive attachment, and Tate action policy through named lower-level owners under `core/tate/`. `core/tate/action_policy.inc` preserves the exact action-member boundary by forwarding to `part_tate_blender_tate_override.inc`, whose rule-sensitive implementation and source-contract anchors remain unchanged. The next Tate migration is to move that implementation behind the named seam after those anchors are migrated, then retire the historical root body. Canonical package: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/tate/package.inc Canonical action seam: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/tate/action_policy.inc C++ textual-include semantics: https://eel.is/c++draft/cpp.include
...
- Tate/provenance composition: `src/trace_engine_v2/core/tate/package.inc`; discard/recovery provenance and proactive attachment own their named lower-level bodies, and `core/tate/action_policy.inc` is the named action composition seam while the historical root body remains the implementation anchor.
...
8. Keep the Tate/provenance package as the sole composition include. The provenance and attachment bodies have named owners, and `core/tate/action_policy.inc` now owns the action composition seam at the identical member boundary. Migrate the rule-sensitive action implementation and its source-contract anchors into the named owner together, then retire the historical `part_tate_blender_tate_override.inc` body.
