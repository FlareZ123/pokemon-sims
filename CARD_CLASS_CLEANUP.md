...
`src/trace_engine_v2/composition/engine_body.inc` is the canonical Engine composition owner. `composition/opening_legacy_stage.inc` owns the historical `part_003.inc` through `part_005.inc` opening chain and its temporary aliases. `composition/banked_tapu_stage.inc` owns the post-search banked-Tapu continuation and its retreat alias. `composition/lock_removal_stage.inc` owns the shared Forest/Field Blower continuation and its lock-removal aliases. `composition/late_legacy_stage.inc` owns the historical `part_014c.inc` through `part_016.inc` execution, registry, reporting chain and its cross-fragment aliases. `composition/opening_engine_overrides.inc` and `composition/post_014a_overrides.inc` remain the lower-level continuation owners inside those named stages.
...
Completed in `cleanup-1786777237845`: extracted the middle banked-Tapu and lock-removal composition blocks from `composition/engine_body.inc` into `composition/banked_tapu_stage.inc` and `composition/lock_removal_stage.inc`. Each stage now owns and asserts its temporary macro contract locally while preserving the exact include order and the existing route-policy owners. C++ textual-include semantics: https://eel.is/c++draft/cpp.include Banked-Tapu route specification: https://github.com/FlareZ123/pokemon-sims/issues/3089 Lock-removal specification: https://github.com/FlareZ123/pokemon-sims/issues/3057

Next composition step: inspect whether `opening_engine_overrides.inc` and `post_014a_overrides.inc` still contain composition-only forwarding that can be collapsed into their named parent stages without moving gameplay policy. Retire only a forwarder whose exact textual boundary, raw-source contracts, and macro lifetime are covered by focused tests.
