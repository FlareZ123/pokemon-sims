...
Completed in `cleanup-1786788022265`: added compile-time contracts at the two remaining macro-lifetime ownership seams. `composition/opening_legacy_stage.inc` now requires the intentional `begin_turn` and `ability_available_for_pokemon` exports after its continuation, while `composition/late_execution_stage.inc` requires every temporary execution wrapper to be released before direct registry/reporting composition. This records the actual ownership exposed by the flattened `engine_body.inc` without moving gameplay policy, DCI/UDP/AMR, K0/K1, Supporter contention, connector domination, or card-resolution behavior. C++ conditional preprocessing: https://eel.is/c++draft/cpp.cond Canonical composition owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/composition/engine_body.inc

Next composition step: inventory `composition/opening_engine_overrides.inc` for one contiguous alias-owned seam whose setup/include/teardown lifetime can move into a named substage without crossing strategy decisions. Keep route decisions in their current owners and preserve the exact historical textual boundary.
