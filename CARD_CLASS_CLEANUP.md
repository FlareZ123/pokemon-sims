...
- `src/trace_engine_v2/core/tate/package.inc` owns the established discard-provenance, Tate attachment, and Tate action override sequence as one composition package. `src/trace_engine_v2/core/tate/attachment_policy.inc` now owns the Tate attachment body, while `src/trace_engine_v2/part_tate_blender_attachment_override.inc` is a thin compatibility include. The remaining provenance and Tate-action bodies stay in their historical fragments until they receive lower-level owners. Canonical attachment owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/tate/attachment_policy.inc Package: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/tate/package.inc
...
- Tate/provenance composition: `src/trace_engine_v2/core/tate/package.inc`; `core/tate/attachment_policy.inc` owns the attachment body, while the provenance and Tate-action fragments remain the next lower-level ownership candidates.
...
8. Keep `core/tate/package.inc` as the sole Tate/provenance composition include. `core/tate/attachment_policy.inc` now owns the former `part_tate_blender_attachment_override.inc` body, and the root attachment path is compatibility-only. Migrate `part_discard_recovery_provenance_override.inc` and `part_tate_blender_tate_override.inc` one at a time next, preserving member order and leaving strategy decisions in Engine. Canonical attachment owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/tate/attachment_policy.inc Compatibility path: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/part_tate_blender_attachment_override.inc
