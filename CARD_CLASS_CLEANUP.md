...
- The banked-Tapu move preserves the existing function body and inline card, rule, policy, and issue URLs. DCI/UDP/AMR, connector domination, K0/K1 timing, route priority, lock admission, action ordering, and readiness behavior stay at their existing owners.
- Next safe follow-up: after CI re-proves the Oricorio parent boundary, switch that parent directly to the canonical `core/routes/oricorio_connector_policy.inc` include and retire its compatibility forwarder. C++ textual-include semantics: https://eel.is/c++draft/cpp.include
