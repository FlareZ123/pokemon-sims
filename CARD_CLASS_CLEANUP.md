...
- `src/trace_engine_v2/core/locks/garbodor_policy.inc` is the canonical Garbodor scenario-timing and Ability-lock policy owner. `GarbodorScenarioPolicy` and `GarbodorAbilityLockPolicy` are explicit final stateless classes, and `composition/engine_body.inc` consumes this canonical owner directly. The historical `src/trace_engine_v2/core/garbodor_lock_policy.inc` path remains only as a compatibility include while source-contract references migrate. Garbodor / Garbotoxin: https://api.pokemontcg.io/v2/cards/xy9-57 Boost Shake: https://api.pokemontcg.io/v2/cards/swsh7-142 Path to the Peak: https://api.pokemontcg.io/v2/cards/swsh6-148 Canonical composition: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/composition/engine_body.inc
...
- `src/trace_engine_v2/core/mysterious_treasure_target_policy.inc` owns the established strategic Mysterious Treasure target priority through `MysteriousTreasureTargetPolicy`. Its `TargetList` alias is the single type owner for the immutable preference table, and callers receive that table by const reference. That route priority is intentionally separate from `SearchConnectorFallbackPolicy`, whose target order applies only after a legal K1 search has started. The remaining `part_009a.inc` route delegates pure physical payload-zone membership to `PayloadZonePolicy`, while DCI/JIT admission, K0/K1 target visibility, connector domination, and action execution remain Engine-owned. Mysterious Treasure: https://api.pokemontcg.io/v2/cards/sm6-113 Payload owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/payload_hand_policy.inc Canonical target policy: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/mysterious_treasure_target_policy.inc
...
- Garbodor scenario and Ability-lock composition: `src/trace_engine_v2/core/locks/garbodor_policy.inc`; `composition/engine_body.inc` now consumes it directly, and `src/trace_engine_v2/core/garbodor_lock_policy.inc` remains only as the temporary source-contract compatibility path. Garbodor: https://api.pokemontcg.io/v2/cards/xy9-57
...
- Mysterious Treasure strategic target priority: `src/trace_engine_v2/core/mysterious_treasure_target_policy.inc` via `MysteriousTreasureTargetPolicy` and its canonical `TargetList` alias.
...
15. Keep `MysteriousTreasureTargetPolicy` limited to strategic route priority and `SearchConnectorFallbackPolicy` limited to legal post-search K1 fallback ordering/traversal. Reuse `MysteriousTreasureTargetPolicy::TargetList` for the immutable preferred-target table rather than repeating its array type. Continue moving the remaining Mysterious Treasure member body out of `part_009a.inc` only when the full continuation can migrate atomically without changing declaration order. C++ textual-include semantics: https://eel.is/c++draft/cpp.include Mysterious Treasure: https://api.pokemontcg.io/v2/cards/sm6-113
