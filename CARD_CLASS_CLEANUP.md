...
`src/trace_engine_v2/core/board_state_policy.inc` owns the shared Active-first mutable/const board traversal, prior-turn evolution timing predicate, canonical `BoardIndex` / `OptionalBoardIndex` vocabulary, shared `BoardAttachmentDestinations` storage, pointer-to-index conversion, and the `active_board_pokemon()` access seam. The board-index mapping is named by `kActiveBoardIndex`, `board_index_for_bench_position()`, and `bench_position_from_board_index()`. Mutable/const pointer lookup reuses the same index-discovery plus index-lookup path, while `board_index_for_pointer()` provides one shared conversion for an already-selected board Pokémon. `src/trace_engine_v2/core/forretress/contract.inc` consumes the shared attachment-destination storage through its card-facing `AttachmentDestinations` alias.

Completed in `cleanup-1786758957236`: centralized Exploding Energy's board-index container type and added the reusable pointer-to-index seam. Completed in `cleanup-1786759283249`: `exploding_energy_regidrago_target_index()` now delegates to `board_index_for_pointer(target_regi())`, Exploding Energy source discovery uses `board_index_matching()`, destination validation and mutation use `board_pokemon_at_index()`, Active detection uses `board_index_is_active()`, Benched self-Knock-Out conversion uses `bench_position_from_board_index()`, and the card-local `pokemon_at_board_index()` member is retired. Completed in `cleanup-1786762777762`: `active_board_pokemon()` now centralizes Active lookup through the same BoardIndex mapping, and `use_exploding_energy_for_setup()` reuses that seam for promotion-only admission, Active source classification, Retreat Cost calculation, post-Ability retreat payment, and the final Active/Bench swap. These are structural changes only; attachment distribution, self-Knock-Out, promotion ranking, DCI/UDP/AMR, K0/K1, connector, readiness, Ability, evolution, and route policy remain at their prior owners.

Next mechanical Forretress step: assess whether the post-Ability retreat-target ranking in `use_exploding_energy_for_setup()` and the self-Knock-Out promotion ranking in `resolve_exploding_energy()` can share a named Regidrago board-ranking primitive without changing either existing deterministic order. Preserve selection, attachment distribution, self-Knock-Out, promotion, retreat payment, and route policy at the Forretress owner.
