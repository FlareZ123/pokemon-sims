...
The namespace-scope Forretress composition has its canonical package at . Public scenario aliases and registry declarations live in , keeping namespace contract ownership separate from runtime mechanics and scenario-family composition at the same verified boundary.  now consumes the canonical package directly at the same post-Engine namespace boundary, and the historical  forwarding path is retired and should not be recreated. Canonical package: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/forretress/package.inc Canonical contract: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/forretress/scenario_contract.inc C++ textual-include semantics: https://eel.is/c++draft/cpp.include
...
- Forretress namespace composition: ; registry declarations remain in , with runtime mechanics in .
...
7. Keep  as the sole namespace-scope Forretress composition owner and  as the registry declaration owner.  consumes the package directly and the historical  forwarding path is retired; do not recreate that compatibility layer.
