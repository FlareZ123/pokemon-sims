...
Next composition step: inspect another remaining root `part_*` one-purpose seam only when its complete macro lifetime or complete function body can move intact into the existing semantic owner at the identical textual boundary. Preserve declaration order, route semantics, and direct source URLs, and do not recreate forwarding-only sequencers.
...
Named Steven route policies live under `src/trace_engine_v2/core/routes/`. The historical #1191/#1204 Gladion/Steven/Treasure continuation now has canonical route ownership in `src/trace_engine_v2/core/routes/gladion_steven_treasure_route_policy.inc`, while the #1595/#2368 Gladion Grass/Turo/Blender continuation now has canonical route ownership in `src/trace_engine_v2/core/routes/gladion_grass_turo_blender_route_policy.inc`. The root `part_issue_1191_gladion_steven_override.inc` and `part_issue_1595_gladion_grass_turo_blender_override.inc` files remain compatibility composition shims at their established `post_014a` alias boundaries. Canonical owners: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/gladion_steven_treasure_route_policy.inc https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/gladion_grass_turo_blender_route_policy.inc Gladion: https://api.pokemontcg.io/v2/cards/sm4-95 Mysterious Treasure: https://api.pokemontcg.io/v2/cards/sm6-113 Regidrago VSTAR: https://api.pokemontcg.io/v2/cards/swsh12-136 Advanced rules: https://github.com/FlareZ123/pokemon-sims/blob/main/EN_advanced_manual-2025-transcription-structured.md
...
Next Steven cleanup step: retire the two remaining compatibility shims only when their complete `play_gladion` alias lifetimes can move intact into `composition/post_014a_overrides.inc` at the same member boundaries. Preserve route admission, DCI/UDP/AMR, Supporter contention, connector domination, hidden-information sequencing, and source URLs. Route-local projected lock checks should reuse `item_locked_on_turn()` whenever their semantics are exactly the shared persistent Item-lock schedule.
...
The same classification owner now centralizes the unmigrated Basic-Pokémon fallback in `is_legacy_basic_pokemon()`. `is_basic()` and broad `is_pokemon()` identity reuse that one list, reducing duplicated legacy card-family membership while leaving registered `CardDefinition` metadata as the first path. Crobat V: https://api.pokemontcg.io/v2/cards/swsh3-104 Pineco: https://api.pokemontcg.io/v2/cards/sv4pt5-1
