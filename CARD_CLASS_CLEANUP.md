...
- `src/trace_engine_v2/core/routes/active_vstar_steven_projection_policy.inc` owns the pure known-deck next-draw risk arithmetic for the active-VSTAR Steven/Crispin/Treasure continuation through final stateless `ActiveVstarStevenProjectionPolicy`. `active_vstar_steven_route_policy.inc` keeps K1 visibility, route admission, lock checks, DCI/JIT timing, card availability, and all physical actions. Steven's Resolve: https://api.pokemontcg.io/v2/cards/sm7-145 Crispin: https://api.pokemontcg.io/v2/cards/sv7-133 Route-risk provenance: https://github.com/FlareZ123/pokemon-sims/issues/1098
...
- Active-VSTAR Steven next-draw projection: `src/trace_engine_v2/core/routes/active_vstar_steven_projection_policy.inc` via `ActiveVstarStevenProjectionPolicy`.
...
- Forretress namespace registry contract: `src/trace_engine_v2/core/forretress/package.inc`; `src/trace_engine_v2/core/forretress/scenario_extension.inc` owns the reusable extension abstraction plus the Garbodor / Boost Shake extension family, and runtime mechanics remain in `core/forretress/runtime.inc`.
...
16. Keep `ActiveVstarStevenProjectionPolicy` limited to the pure #1098 next-draw risk arithmetic. Any future cleanup of this route must leave K1 visibility, action-specific locks, DCI/JIT, card availability, route admission, and physical execution in `active_vstar_steven_route_policy.inc`. Steven's Resolve: https://api.pokemontcg.io/v2/cards/sm7-145 Crispin: https://api.pokemontcg.io/v2/cards/sv7-133
