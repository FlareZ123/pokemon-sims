...
- This consolidation is mechanical. It preserves the exact function bodies, macro lifetimes, helper include, direct card/rule/policy/issue URLs, and declaration order. Future Arven cleanup should continue from `composition/post_014a_overrides.inc` and avoid recreating one-owner `part_*.inc` fragments.
...
- Proven parent boundary: `src/trace_engine_v2/composition/post_014a_overrides.inc` is the live parent of `core/oricorio_connector_policy.inc`, immediately after the empty-deck search wrapper chain. The substantive Oricorio policy remains owned by `src/trace_engine_v2/core/routes/oricorio_connector_policy.inc`; the compatibility forwarder remains in place until that exact parent include is switched. Oricorio / Vital Dance: https://api.pokemontcg.io/v2/cards/sm2-55 Regidrago VSTAR / Apex Dragon: https://api.pokemontcg.io/v2/cards/swsh12-136 Canonical owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/oricorio_connector_policy.inc
- The banked-Tapu and Oricorio seam cleanups preserve the existing route bodies, inline card/rule/policy/issue URLs, DCI/UDP/AMR, connector domination, K0/K1 timing, route priority, lock admission, action ordering, readiness behavior, and Retreat legality.
- Next safe follow-up: at the proven `composition/post_014a_overrides.inc` boundary, replace `#include "../core/oricorio_connector_policy.inc"` with `#include "../core/routes/oricorio_connector_policy.inc"`, then retire `src/trace_engine_v2/core/oricorio_connector_policy.inc`. C++ textual-include semantics: https://eel.is/c++draft/cpp.include
...
## Cleanup wave 2026-08-14 root card seam retirement

- `src/trace_engine_v2/composition/engine_body.inc` directly composes `core/card_catalog.inc` and `core/card_classification.inc`. The root `part_001.inc` compatibility shim is retired. `part_000.inc` remains as a minimal compatibility seam because `tests/generate_unified_tests.py` reads that path while collecting source headers for unified regression generation. Canonical catalog: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/card_catalog.inc Canonical classification: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/card_classification.inc Unified test generator: https://github.com/FlareZ123/pokemon-sims/blob/main/tests/generate_unified_tests.py
- The cleanup removes the legacy source-anchor and inactive source-mirror scaffolding while preserving the live `part_000.inc` source contract, card metadata, classification behavior, DCI/UDP/AMR, connector domination, K0/K1 state, route ordering, and printed resolution. Engine composition: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/composition/engine_body.inc C++ textual-include semantics: https://eel.is/c++draft/cpp.include
- Retire `part_000.inc` only after unified-test header discovery is migrated to the canonical owner. Continue validating strict compilation, the regression suites, permanent `--simulate-this` audits, and the paired T2/T3 matrix before merge.
