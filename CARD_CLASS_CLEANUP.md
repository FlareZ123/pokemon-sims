...
- `src/cards/card_definition.hpp` owns intrinsic exact-print facts: name, print ID, Trainer subtype, stage/type, Retreat Cost, Rule Box/Pokémon V/ACE SPEC/Basic Energy flags, and direct source URL.
...
- `src/trace_engine_v2/core/card_catalog.inc` owns unmigrated name and intrinsic-classification compatibility. Registry lookup remains first and canonical.
...
Regidrago VSTAR already owns exact Silver Tempest 136/195 metadata beside Regidrago V, is explicitly registered, and has focused metadata/parity coverage. Exact prints: https://api.pokemontcg.io/v2/cards/swsh12-135 https://api.pokemontcg.io/v2/cards/swsh12-136
...
The remaining root `part_*` seams should be retired only when one of these is proven:

- the file is forwarding-only and every generator/source-contract consumer can reference its canonical owner directly; or
- its complete function body or macro lifetime can move intact to the identical textual boundary.

Do not split a semantic owner merely to make a composition file shorter.

### Current composition next step

The issue-1393 held-Crispin completion helper has canonical semantic ownership at `src/trace_engine_v2/core/routes/crispin_supported_route_policy.inc`. The root `part_issue_1393_crispin_route_helper.inc` remains a minimal compatibility seam until every source-contract or generator consumer references the canonical route owner directly. Crispin: https://api.pokemontcg.io/v2/cards/sv7-133 Advanced procedure: https://github.com/FlareZ123/pokemon-sims/blob/main/EN_advanced_manual-2025-transcription-structured.md

## Payload policy cleanup

`src/trace_engine_v2/core/payload_hand_policy.inc` is the canonical Dragon-payload query owner. Reuse `PayloadZonePolicy::contains()`, `contains_card()`, `contains_matching()`, `count()`, and `count_matching()` only when semantics match exactly. Preserve physical-order selection where order is observable and preserve explicit strategic ordering where preference is required.
...
`src/trace_engine_v2/core/forretress/runtime.inc` now owns the complete Forretress runtime, including the contiguous printed Exploding Energy resolver, Forretress-stack discard transition, board-index target adapter, immediate post-KO promotion, setup orchestration, and search connectors. The forwarding split `src/trace_engine_v2/core/forretress/exploding_energy_runtime.inc` is retired. This keeps the printed Ability transition beside its only runtime caller without changing state mutation, attachment distribution, Knock Out handling, promotion ranking, or source URLs. Forretress ex: https://api.pokemontcg.io/v2/cards/sv4pt5-2 Core Ability/search/attachment/Knock Out procedure: https://www.pokemon.com/us/pokemon-tcg/rules Official February 2026 ruling: https://professorprogram.pokemon.com/news/11473085
...
Named Steven route policies live under `src/trace_engine_v2/core/routes/`. Reuse `resolve_gladion_prize_exchange()` only after a caller has already completed legal reveal and target selection with identical semantics. Reuse `item_locked_on_turn()` for shared persistent Item-lock timing.
