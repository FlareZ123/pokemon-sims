...
Next mechanical Forretress step: route the remaining Active-specific source questions in `use_exploding_energy_for_setup()` through the shared board-index vocabulary. After that, extract the post-Ability retreat-target ranking only if its current comparator can be expressed over `visit_board_pokemon_in_zones()` with byte-for-byte deterministic tie ordering. Add a named best-match board query only when that equivalence is demonstrated. Preserve selection, attachment distribution, self-Knock-Out, promotion, retreat payment, and route policy at the Forretress owner.
