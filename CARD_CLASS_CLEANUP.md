...
- The direct banked-Tapu composition preserves the existing function body, alias lifetime, card/rule/policy URLs, declaration order, DCI/UDP/AMR, connector domination, K0/K1 timing, route priority, lock admission, action ordering, and readiness behavior.
- Next safe follow-up: after CI re-proves the Oricorio parent boundary, switch that parent directly to the canonical `core/routes/oricorio_connector_policy.inc` include and retire the compatibility forwarder. C++ textual-include semantics: https://eel.is/c++draft/cpp.include
