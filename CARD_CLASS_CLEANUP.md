...
## Cleanup wave 2026-08-13 registry-contract ownership

- Compile-time bridge assertions for migrated card ids, canonical print ids, and display names now live with the explicit registry in `src/cards/card_registry.hpp`. The trace engine no longer owns a second copy of those contracts in `core/card_catalog.inc`. Registry owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/cards/card_registry.hpp
- Keep future exact-print bridge assertions adjacent to the registered definition that owns the intrinsic metadata, with the direct card-data URL on the relevant assertion. Quick Ball: https://api.pokemontcg.io/v2/cards/swsh1-179 Guzma & Hala: https://api.pokemontcg.io/v2/cards/sm12-229 Arven: https://api.pokemontcg.io/v2/cards/sv1-166 Dawn: https://api.pokemontcg.io/v2/cards/me2-87 Powerglass: https://api.pokemontcg.io/v2/cards/sv6pt5-63
- This ownership cleanup does not move printed resolution, route admission, DCI/UDP/AMR, connector domination, K0/K1, or matchup policy. Those responsibilities remain at their existing simulator owners until a separately validated migration proves the live resolution boundary.
