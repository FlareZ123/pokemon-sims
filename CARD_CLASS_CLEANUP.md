...
The issue-1199 Steven package body now has its canonical organized owner at `src/trace_engine_v2/core/routes/issue_1199_steven_package_policy.inc`. The historical `src/trace_engine_v2/part_issue_1199_steven_package_override.inc` path is a thin compatibility include so any remaining source-contract consumers preserve the identical member boundary while references migrate. Keep K0/K1 admission, DCI/JIT timing, Supporter contention, route priority, and physical card actions in the route owner. Steven's Resolve: https://api.pokemontcg.io/v2/cards/sm7-145 Mysterious Treasure: https://api.pokemontcg.io/v2/cards/sm6-113 Canonical owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/issue_1199_steven_package_policy.inc
...
3. Continue retiring other composition-only `part_*steven*` forwarders when a complete function body or macro lifetime can move at the identical textual boundary. The issue-1199 body now lives in `core/routes/issue_1199_steven_package_policy.inc`; migrate remaining references to its canonical path before retiring the root compatibility include. The issue-3221 and issue-3222 root semantic forwarders are complete and should not be recreated.
