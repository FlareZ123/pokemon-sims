...
## Cleanup wave 2026-08-14 compatibility seam retirement

- This cleanup branch retires the root-level `src/trace_engine_v2/core/steven_package_policy.inc` forwarding seam. Steven package policy remains owned by `src/trace_engine_v2/core/routes/steven_package_policy.inc`; future composition should include that canonical owner directly when needed. Canonical owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/steven_package_policy.inc C++ textual-include semantics: https://eel.is/c++draft/cpp.include
- This cleanup branch also retires `src/trace_engine_v2/core/regidrago_line_helpers.inc`, whose only responsibility was forwarding to the already-canonical intrinsic identity predicates in `src/trace_engine_v2/core/card_classification.inc`. Regidrago V: https://api.pokemontcg.io/v2/cards/swsh12-135 Regidrago VSTAR: https://api.pokemontcg.io/v2/cards/swsh12-136 Canonical classification owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/card_classification.inc
- These removals are intended to be ownership-only. They do not change DCI/UDP/AMR, connector domination, K0/K1 timing, card resolution, route admission, lock behavior, action ordering, or readiness. Strict Release compilation, representative `--simulate-this` audits, the full regression suite, sanitizer checks, and the paired T2/T3 matrix remain the merge gate.
