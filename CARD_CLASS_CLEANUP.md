...
## Cleanup wave 2026-08-14 Steven/Vessel route ownership

- The issue-3202 Steven/Vessel route policy now has one canonical implementation owner at `src/trace_engine_v2/core/routes/issue_3202_steven_vstar_vessel_policy.inc`. Steven's Resolve: https://api.pokemontcg.io/v2/cards/sm7-145 Earthen Vessel: https://api.pokemontcg.io/v2/cards/sv4-163 Regidrago VSTAR: https://api.pokemontcg.io/v2/cards/swsh12-136
- `src/trace_engine_v2/part_issue_3202_steven_vstar_vessel_semantic_override.inc` is reduced to a thin compatibility forwarder because `composition/opening_engine_overrides.inc` still consumes that historical boundary. The route body, macro lifetime, printed-card URLs, K1/JIT/lock policy, and declaration order are unchanged. Confirmed behavior: https://github.com/FlareZ123/pokemon-sims/issues/3202 C++ textual-include semantics: https://eel.is/c++draft/cpp.include
- Next safe follow-up: once the `opening_engine_overrides.inc` member boundary is re-proven independently, include the canonical `core/routes/` owner directly and retire the historical forwarder. Keep that later edit mechanical and require the same strict compilation, sanitizer suite, representative `--simulate-this` audits, and fixed-seed T2/T3 matrix before merge.
