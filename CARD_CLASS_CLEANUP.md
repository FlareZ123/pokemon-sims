...
- `src/trace_engine_v2/core/tate/package.inc` owns the established discard-provenance, Tate attachment, and Tate action override order. `core/tate/discard_recovery_provenance.inc`, `core/tate/attachment_policy.inc`, and `core/tate/action_policy.inc` are now the live named lower-level owners for all three bodies. The historical root action path is retained only as a compatibility forwarding include while source-contract anchors migrate: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/tate/package.inc
...
The complete search-connector preparation and post-search fallback helpers have one named owner at `src/trace_engine_v2/core/routes/search_connector_helpers.inc`. The owner now centralizes the repeated K1 physical deck scan while preserving separate ordered target sets for Mysterious Treasure, Quick Ball, and Ultra Ball. `part_008b.inc` includes that owner directly after the Heavy Ball policy. The next numbered boundary begins with the Mysterious Treasure route in `part_009a.inc`; its current member closes at the beginning of `part_009b1.inc`, so any extraction must move that continuation atomically at the same declaration boundary. C++ textual-include semantics: https://eel.is/c++draft/cpp.include Mysterious Treasure: https://api.pokemontcg.io/v2/cards/sm6-113 Quick Ball: https://api.pokemontcg.io/v2/cards/swsh1-179 Ultra Ball: https://api.pokemontcg.io/v2/cards/swsh12pt5-146

The Tate package now composes discard/recovery provenance, proactive attachment, and the Tate action override from `core/tate/discard_recovery_provenance.inc`, `core/tate/attachment_policy.inc`, and `core/tate/action_policy.inc` at the same member positions. The historical `part_tate_blender_tate_override.inc` path is reduced to a compatibility forwarding include for source-contract anchors and can be retired after those references migrate. Canonical package: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/tate/package.inc C++ textual-include semantics: https://eel.is/c++draft/cpp.include
...
- Tate/provenance composition: `src/trace_engine_v2/core/tate/package.inc`; discard/recovery provenance, proactive attachment, and Tate action policy now compose from named lower-level owners under `core/tate/`.
...
8. Keep the Tate/provenance package as the sole composition include. The provenance, attachment, and action bodies now have named owners under `core/tate/`; migrate remaining source-contract anchors away from historical Tate root paths, then retire those forwarding paths.
