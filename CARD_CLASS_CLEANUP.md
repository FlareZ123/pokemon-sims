...
- `src/trace_engine_v2/core/board_state_policy.inc` owns reusable board traversal and board-index queries. `src/trace_engine_v2/core/guzma_hala_policy.inc` now delegates its whole-board Regidrago holder query, open Pokémon V Tool-slot query, and Bench-only projected Double Dragon Energy scan to this owner while retaining Guzma & Hala route admission and projection policy. Canonical board owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/board_state_policy.inc Guzma & Hala policy: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/guzma_hala_policy.inc
...
6. Continue replacing route-local board traversals with `core/board_state_policy.inc` only when first-match/order semantics are identical. The Quick Ball/Latias completion route and Guzma & Hala whole-board/Bench projections are migrated; preserve each route's JIT, evolution, connector, and target-admission decisions in its strategy owner.
