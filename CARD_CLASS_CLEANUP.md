...
- `src/trace_engine_v2/core/routes/pokemon_search_items/pokemon_communication.inc` is the canonical Pokémon Communication Engine route owner. The historical `src/trace_engine_v2/part_pokemon_communication.inc` path is now a compatibility include at the same textual member boundary so route admission, DCI/UDP/AMR scoring, K0/K1 visibility, Supporter contention, target selection, and printed resolution remain in one implementation. Pokémon Communication: https://api.pokemontcg.io/v2/cards/sm9-152 Canonical owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/pokemon_search_items/pokemon_communication.inc
...
21. Keep Pokémon Communication route logic in `core/routes/pokemon_search_items/pokemon_communication.inc`. Migrate remaining source-contract references from `part_pokemon_communication.inc` to the canonical owner, then retire the root compatibility include only after repository-wide references are gone. Preserve the established Engine member boundary and printed reveal-return-search-shuffle order. Pokémon Communication: https://api.pokemontcg.io/v2/cards/sm9-152 C++ textual-include semantics: https://eel.is/c++draft/cpp.include
