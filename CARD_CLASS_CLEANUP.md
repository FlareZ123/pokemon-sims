...
The namespace-scope Forretress composition has a canonical package at `src/trace_engine_v2/core/forretress/package.inc`. `composition/engine_body.inc` composes that canonical package directly. The historical `src/trace_engine_v2/part_forretress_ex_combo.inc` forwarding path is retired after direct composition ownership was established; merge remains subject to the validation gate below. The package keeps the reusable runtime, public scenario forward declarations, shared scenario extension, and Garbodor scenario family at the same verified namespace boundary while preserving their textual order. Canonical package: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/forretress/package.inc C++ textual-include semantics: https://eel.is/c++draft/cpp.include

The issue-3221 K0 Steven/Brilliant Blender route remains owned by `src/trace_engine_v2/core/routes/k0_steven_blender_semantic_policy.inc`. The historical `src/trace_engine_v2/part_issue_3221_k0_steven_blender_semantic_override.inc` root forwarding path is retired because canonical engine composition already owns the route directly; merge remains subject to the validation gate below. Brilliant Blender: https://api.pokemontcg.io/v2/cards/sv8-164 Steven's Resolve: https://api.pokemontcg.io/v2/cards/sm7-145 Regidrago VSTAR: https://api.pokemontcg.io/v2/cards/swsh12-136 Advanced procedure: https://github.com/FlareZ123/pokemon-sims/blob/main/EN_advanced_manual-2025-transcription-structured.md

The issue-3222 Steven/Brilliant Blender semantic route remains owned by `src/trace_engine_v2/core/routes/steven_blender_semantic_policy.inc`. The historical `src/trace_engine_v2/part_issue_3222_steven_blender_semantic_override.inc` root forwarding path is retired because canonical engine composition already owns the route directly; merge remains subject to the validation gate below. Brilliant Blender: https://api.pokemontcg.io/v2/cards/sv8-164 Steven's Resolve: https://api.pokemontcg.io/v2/cards/sm7-145 Advanced procedure: https://github.com/FlareZ123/pokemon-sims/blob/main/EN_advanced_manual-2025-transcription-structured.md
...
1. Keep `composition/steven/blender_overrides.inc` and `core/routes/steven/package.inc` as the canonical Steven organization boundaries. Remove their remaining forwarding paths only after repository-wide and source-contract references are proven gone.
2. Continue retiring composition-only `part_*steven*` forwarders whose engine consumers already compose their canonical owners directly. Treat the retired issue-3221 and issue-3222 root seams as the reference pattern.
...
6. Keep Forretress reusable scenario and runtime ownership under `core/forretress/`, with `core/forretress/package.inc` as the namespace-scope composition owner. Treat the retired `part_forretress_ex_combo.inc` seam as the reference pattern for removing composition-only forwarders after direct ownership is established.
