...
The Heavy Ball and shared search-connector member bundle is now composed through `src/trace_engine_v2/core/routes/search_connector_package.inc`. The package preserves the historical Hisuian Heavy Ball then connector-helper order and requires the caller-owned `play_mysterious_treasure` and `play_heavy_ball` aliases to survive the package boundary. `src/trace_engine_v2/part_008b.inc` is now a thin historical composition seam. Hisuian Heavy Ball: https://api.pokemontcg.io/v2/cards/swsh10-146 Mysterious Treasure: https://api.pokemontcg.io/v2/cards/sm6-113 Quick Ball: https://api.pokemontcg.io/v2/cards/swsh1-179 Ultra Ball: https://api.pokemontcg.io/v2/cards/swsh12pt5-146 Canonical package: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/search_connector_package.inc C++ textual-include semantics: https://eel.is/c++draft/cpp.include

The complete search-connector preparation and post-search fallback helpers remain owned by `src/trace_engine_v2/core/routes/search_connector_helpers.inc`. `SearchConnectorFallbackPolicy` centralizes the printed legal fallback orders and shared first-available K1 traversal for Mysterious Treasure, Quick Ball, and Ultra Ball. Mysterious Treasure's separate strategic priority remains owned by `core/mysterious_treasure_target_policy.inc`. The new package only centralizes textual composition and does not move DCI/UDP/AMR, K0/K1 admission, or route-specific target choice out of Engine strategy. C++ textual-include semantics: https://eel.is/c++draft/cpp.include Mysterious Treasure: https://api.pokemontcg.io/v2/cards/sm6-113 Quick Ball: https://api.pokemontcg.io/v2/cards/swsh1-179 Ultra Ball: https://api.pokemontcg.io/v2/cards/swsh12pt5-146
...
- Heavy Ball/search connector composition: `src/trace_engine_v2/core/routes/search_connector_package.inc`, preserving the Heavy Ball then helper order and the caller-owned Treasure/Heavy Ball aliases.
...
16. Migrate source-contract references from `part_008b.inc` to `core/routes/search_connector_package.inc`. Once repository-wide references are gone, compose the named package directly from `composition/opening_engine_overrides.inc` at the identical macro boundary and retire the historical `part_008b.inc` seam.
