...
`src/trace_engine_v2/composition/engine_body.inc` is the canonical Engine composition owner. `composition/opening_engine_overrides.inc` owns the early Supporter/VSTAR continuation. `composition/post_014a_overrides.inc` owns late-search composition. `composition/opening_legacy_stage.inc` now owns the historical `part_003.inc` through `part_005.inc` opening chain and its temporary aliases. `composition/middle_policy_stage.inc` owns the contiguous banked-Tapu replacement plus lock-removal composition sequence. `composition/late_legacy_stage.inc` now owns the historical `part_014c.inc` through `part_016.inc` execution, registry, reporting chain and its cross-fragment aliases.
...
Next composition step: inventory `composition/middle_policy_stage.inc` for a smaller compatibility seam only when source-contract coverage can assert its exact textual boundary. Keep banked-Tapu route decisions in `core/routes/banked_tapu_retreat_policy.inc`, lock-removal decisions in their existing policy owners, and do not split the alias setup from the include/teardown sequence it protects.
