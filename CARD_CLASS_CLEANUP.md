...
## Cleanup wave 2026-08-14 Oricorio direct-owner follow-up

- Completed: `src/trace_engine_v2/composition/post_014a_overrides.inc` now includes `src/trace_engine_v2/core/routes/oricorio_connector_policy.inc` directly at the same proven late-search member boundary, and the former `src/trace_engine_v2/core/oricorio_connector_policy.inc` compatibility forwarder is retired. Canonical owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/oricorio_connector_policy.inc C++ textual-include semantics: https://eel.is/c++draft/cpp.include
- The direct include preserves the existing Oricorio route body, macro lifetime, declaration order, DCI/UDP/AMR, connector domination, K0/K1 timing, Energy-axis policy, and readiness behavior. Oricorio / Vital Dance: https://api.pokemontcg.io/v2/cards/sm2-55 Regidrago VSTAR / Apex Dragon: https://api.pokemontcg.io/v2/cards/swsh12-136
- `is_regidrago_v_line(Card card)` keeps the shared Regidrago V/VSTAR identity predicate behavior unchanged while removing a redundant top-level `const` on its by-value parameter. Regidrago V: https://api.pokemontcg.io/v2/cards/swsh12-135 Regidrago VSTAR: https://api.pokemontcg.io/v2/cards/swsh12-136
- Next safe follow-up: use the new cohesive `core/forretress/runtime.inc` and `core/forretress/scenarios.inc` ownership introduced on current `main` when evaluating whether `core/forretress_combo_contract.inc` still needs its compatibility boundary. Keep any later change type-only until CI establishes equivalent composition.
