...
## Cleanup wave 2026-08-14 shared classifier and Steven route package

- `src/trace_engine_v2/core/card_classification.inc` now owns `is_regidrago_v_line()` beside the simulator's other intrinsic Pokémon classifiers, with exact Regidrago V and Regidrago VSTAR sources kept beside that predicate. Regidrago V: https://api.pokemontcg.io/v2/cards/swsh12-135 Regidrago VSTAR: https://api.pokemontcg.io/v2/cards/swsh12-136 Canonical classifier: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/card_classification.inc
- `src/trace_engine_v2/core/regidrago_line_helpers.inc` remains a compatibility seam for historical include sites and delegates classification ownership to `card_classification.inc`. Future cleanup may retire the seam only after every parent include is proven to load classification directly.
- `src/trace_engine_v2/core/routes/steven_package_policy.inc` is now the canonical ordered owner for the issue-1745, issue-1771, issue-1772, and issue-2622 Steven continuations. `src/trace_engine_v2/core/steven_package_policy.inc` remains a forwarding compatibility boundary. Steven's Resolve: https://api.pokemontcg.io/v2/cards/sm7-145 C++ textual-include semantics: https://eel.is/c++draft/cpp.include Canonical route package: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/steven_package_policy.inc
- These ownership changes are behavior-preserving. DCI/UDP/AMR, connector domination, K0/K1 timing, Supporter contention, route priority, lock admission, action ordering, and readiness stay at their existing policy owners. Advanced rules source: https://github.com/FlareZ123/pokemon-sims/blob/main/EN_advanced_manual-2025-transcription-structured.md
