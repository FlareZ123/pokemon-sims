...
The issue-1393 held-Crispin completion route remains owned by `src/trace_engine_v2/core/routes/crispin_supported_route_policy.inc`. Its K1 Basic Energy availability is now captured once as an explicit route snapshot, and its one- or two-attachment Apex projection reuses the canonical attachment and Apex-payment helpers through one named route helper. Keep Supporter contention, strict-JIT admission, K1 visibility, payload-outlet selection, and action execution in Engine strategy. Crispin: https://api.pokemontcg.io/v2/cards/sv7-133 Regidrago VSTAR / Apex Dragon: https://api.pokemontcg.io/v2/cards/swsh12-136 K1 and decision policy: https://github.com/FlareZ123/pokemon-sims/blob/main/docs/POLICY_DECISIONS.md
...
9. Prefer named pure-projection members over route-local anonymous projections when a projection is reused or has a distinct policy contract. The issue-1393 Crispin route now applies this to its K1 Basic Energy snapshot and Apex completion projection; keep later route cleanup on the same boundary without moving Supporter, JIT, or payload decisions out of Engine.
