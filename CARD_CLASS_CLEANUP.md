...
- Forretress namespace composition: `src/trace_engine_v2/core/forretress/package.inc`; registry declarations remain in `src/trace_engine_v2/core/forretress/scenario_contract.inc`, with runtime mechanics in `core/forretress/runtime.inc`.
...
7. Keep `core/forretress/package.inc` as the sole namespace-scope Forretress composition owner and `core/forretress/scenario_contract.inc` as the registry declaration owner. `part_014c.inc` consumes the package directly and the historical `part_forretress_ex_combo.inc` forwarding path is retired; do not recreate that compatibility layer.
