...
- `src/trace_engine_v2/core/adapters/card_context_adapter.hpp` owns the trace-engine bridge for reusable card effects. `src/trace_engine_v2/core/card_catalog.inc` consumes that canonical adapter directly; the historical `src/trace_engine_v2/core/card_context_adapter.hpp` forwarding include is retired and should not be recreated.
...
4. Keep direct trace-engine bridge consumers on `core/adapters/card_context_adapter.hpp`. The historical `core/card_context_adapter.hpp` forwarding include is retired and should not be recreated. New bridge construction should use `CardContextAdapterCallbacks`.
