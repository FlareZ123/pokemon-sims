...
- `src/trace_engine_v2/core/locks/garbodor_policy.inc` is the canonical Garbodor scenario-timing and Ability-lock policy owner. `GarbodorScenarioPolicy` and `GarbodorAbilityLockPolicy` are explicit final stateless classes. `composition/engine_body.inc` and `src/regidrago_sim.cpp` now reference the canonical organized path directly. The historical `src/trace_engine_v2/core/garbodor_lock_policy.inc` path remains only as a residual compatibility include while repository-wide source-contract references are checked. Garbodor / Garbotoxin: https://api.pokemontcg.io/v2/cards/xy9-57 Boost Shake: https://api.pokemontcg.io/v2/cards/swsh7-142 Path to the Peak: https://api.pokemontcg.io/v2/cards/swsh6-148
...
- `src/trace_engine_v2/core/deck_knowledge.inc` owns reusable copy arithmetic after visibility is resolved. `KnowledgeCopyPolicy` owns one `PublicZoneCopyCounts` value for the hand/discard/attached inputs so K0 arithmetic does not duplicate anonymous positional count tuples. Hidden-zone visibility and route admission remain Engine concerns. K0/K1 contract: https://github.com/FlareZ123/pokemon-sims/blob/main/docs/POLICY_DECISIONS.md#knowledge-states
...
- `src/trace_engine_v2/core/routes/active_vstar_steven_route_policy.inc` now co-locates the pure known-deck next-draw risk arithmetic with the active-VSTAR Steven/Crispin/Treasure route that consumes it through final stateless `ActiveVstarStevenProjectionPolicy`. The separate `active_vstar_steven_projection_policy.inc` forwarding fragment is retired. Engine still owns K1 visibility, route admission, lock checks, DCI/JIT timing, card availability, and all physical actions. Steven's Resolve: https://api.pokemontcg.io/v2/cards/sm7-145 Crispin: https://api.pokemontcg.io/v2/cards/sv7-133 Route-risk provenance: https://github.com/FlareZ123/pokemon-sims/issues/1098
...
- `src/trace_engine_v2/core/forest_field_blower_policy.inc` keeps the issue-3057 Forest/Field Blower route in Engine while `PinecoEntryTimingPolicy` owns only the pure prior-turn/current-turn Pineco entry predicates reused by that route. Forest of Vitality: https://api.pokemontcg.io/v2/cards/me1-117 Pineco: https://api.pokemontcg.io/v2/cards/sv4pt5-1
...
- Garbodor scenario and Ability-lock composition: `src/trace_engine_v2/core/locks/garbodor_policy.inc`; Engine composition and the root source registry now use this canonical path directly, leaving `src/trace_engine_v2/core/garbodor_lock_policy.inc` only as a residual compatibility seam pending repository-wide source-contract verification. Garbodor: https://api.pokemontcg.io/v2/cards/xy9-57
...
- Active-VSTAR Steven next-draw projection: `src/trace_engine_v2/core/routes/active_vstar_steven_route_policy.inc` via co-located `ActiveVstarStevenProjectionPolicy`.
...
- Public-zone copy-count values: `src/trace_engine_v2/core/deck_knowledge.inc` via `KnowledgeCopyPolicy::PublicZoneCopyCounts`; Engine callers still decide K0/K1 visibility before constructing the value.
- Pineco entry-turn classification for issue-3057: `src/trace_engine_v2/core/forest_field_blower_policy.inc` via `PinecoEntryTimingPolicy`; route admission, connector choice, K1/DCI state, and physical Stadium/Item actions remain Engine-owned.
...
6. Continue replacing route-local board traversals with `core/board_state_policy.inc` only when first-match/order semantics are identical. The Quick Ball/Latias completion route and Professor Burnet Bench-presence queries are migrated; preserve route-local JIT, evolution, connector, and action decisions.
...
20. Engine composition and the root source registry now use `core/locks/garbodor_policy.inc` directly. Verify any remaining repository-wide source-contract references, then retire `core/garbodor_lock_policy.inc` when none remain. Keep scenario timing and Ability-lock composition in the final stateless policy classes; Engine retains live scenario/state reads and shared lock-removal state. Garbodor / Garbotoxin: https://api.pokemontcg.io/v2/cards/xy9-57 Boost Shake: https://api.pokemontcg.io/v2/cards/swsh7-142 Path to the Peak: https://api.pokemontcg.io/v2/cards/swsh6-148
21. Keep K0/K1 visibility decisions outside `KnowledgeCopyPolicy`. Pass resolved public-zone counts through `PublicZoneCopyCounts` when several arithmetic helpers share the same hand/discard/attached tuple, and avoid adding Engine state access to the policy class.
22. Reuse `PinecoEntryTimingPolicy` for issue-3057 prior-turn/current-turn entry classification only. Keep Forest availability, Forretress route admission, K1/AMR/DCI checks, connector domination, and physical Item/Stadium actions in the existing Engine route. Forest of Vitality: https://api.pokemontcg.io/v2/cards/me1-117 Pineco / Forretress ex: https://api.pokemontcg.io/v2/cards/sv4pt5-1 https://api.pokemontcg.io/v2/cards/sv4pt5-2
