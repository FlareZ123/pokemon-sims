...
- `src/trace_engine_v2/core/routes/earthen_vessel_vstar_window_policy.inc` owns the Earthen Vessel / Regidrago VSTAR strict-JIT window and K0/K1 preservation policy formerly stored in the historical root override. Earthen Vessel: https://api.pokemontcg.io/v2/cards/sv4-163 Regidrago VSTAR: https://api.pokemontcg.io/v2/cards/swsh12-136
...
11. Continue moving complete root `part_*` route bodies into `core/routes/` one owner at a time. Leave a forwarding seam at the historical path until source-contract references are migrated, and preserve the exact Engine member boundary plus all rule and issue provenance during each move.
