...
`src/trace_engine_v2/core/forretress/scenario_extension.inc` now owns the reusable namespace-scope `ScenarioExtension` append/lookup class. `part_forretress_ex_combo.inc` keeps only the historical scenario declarations, Garbodor row data, and public extension entry points, then composes the shared class from the Forretress core folder. This removes generic container traversal from the root feature boundary while preserving scenario order, lookup values, and owned-range lifetime. Canonical owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/forretress/scenario_extension.inc C++ textual include semantics: https://eel.is/c++draft/cpp.include C++ object lifetime rules: https://eel.is/c++draft/basic.life

Next mechanical Forretress step: inventory remaining orchestration in `runtime.inc` and adjacent root route fragments for another complete semantic boundary. Keep feature-specific scenario rows at the namespace composition boundary while reusable append/lookup mechanics stay under `core/forretress/`. Reuse board-policy classifiers only where semantics match exactly. Preserve state-count queries, entry-turn evolution timing, route ordering, attachment distribution, retreat planning, and strategic ranking at their existing owners. Forest of Vitality: https://api.pokemontcg.io/v2/cards/me1-117 Core evolution rules: https://www.pokemon.com/us/pokemon-tcg/rules

## Steven route cleanup

Named Steven route policies live under `src/trace_engine_v2/core/routes/`. `core/routes/gladion_steven_route_policy.inc` now owns the shared `resolve_gladion_prize_exchange()` state transition together with the complete #1191/#1204 Gladion, Steven's Resolve, and Mysterious Treasure continuation. Its route-local helpers use semantic names while the historical #1204 helper names remain thin source-contract wrappers for unified tests. The historical `part_issue_1191_gladion_steven_override.inc` path is a minimal compatibility seam that includes the canonical owner under the same parent `play_gladion` alias. Admission, target choice, hidden-information sequencing, DCI/JIT policy, trace text, and direct rule/card URLs remain with the canonical route logic. Steven's Resolve: https://api.pokemontcg.io/v2/cards/sm7-145 Gladion: https://api.pokemontcg.io/v2/cards/sm4-95 Mysterious Treasure: https://api.pokemontcg.io/v2/cards/sm6-113 Advanced rules: https://github.com/FlareZ123/pokemon-sims/blob/main/EN_advanced_manual-2025-transcription-structured.md Canonical owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/gladion_steven_route_policy.inc Test contract: https://github.com/FlareZ123/pokemon-sims/blob/main/tests/issue_1204_gladion_treasure_tests.cpp
...
Next Steven cleanup step: retire `part_issue_1191_gladion_steven_override.inc` once the live post-search composition consumer and source-contract tooling can include `core/routes/gladion_steven_route_policy.inc` directly at the identical `play_gladion` wrapper boundary. Continue migrating duplicated Gladion Prize-exchange mutations to `resolve_gladion_prize_exchange()` only when reveal and target-selection semantics match exactly. Keep the new `composition/steven_blender_overrides.inc` boundary stable until its inner route seams can be retired without changing macro lifetime.
