...
The Quick Ball card-class bridge now constructs `CardContext` through a named `CardContextAdapterCallbacks` bundle, and the positional adapter overload is retired. Next adapter step: migrate any remaining direct includes of `src/trace_engine_v2/core/card_context_adapter.hpp` to `core/adapters/card_context_adapter.hpp`, then delete the forwarding compatibility include after repository-wide consumers and source-contract tooling are gone. Preserve callback behavior and card/strategy ownership while doing so.
