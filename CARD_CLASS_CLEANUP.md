...
`src/trace_engine_v2/part_forretress_ex_combo.inc` owns the Garbodor scenario extension directly beside the `core/forretress/runtime.inc` include. Its extension mechanics now use local generic helpers for range append and label lookup, keeping scenario ordering and lookup behavior in one reusable shape without introducing another `.inc` file. Scenario owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/part_forretress_ex_combo.inc Garbodor: https://api.pokemontcg.io/v2/cards/xy9-57 Boost Shake: https://api.pokemontcg.io/v2/cards/swsh7-142 Scenario specification: https://github.com/FlareZ123/pokemon-sims/issues/2808
...
The Forretress contract now names `BoardIndex`, `OptionalBoardIndex`, and `AttachmentDestinations` as the single board-index vocabulary and marks board-index lookup as a query. Next mechanical Forretress step: migrate raw board-index spellings in `src/trace_engine_v2/core/forretress/runtime.inc` to those existing aliases. Keep that edit type-only. Preserve Exploding Energy logic and its source-bound rulings. Current runtime: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/forretress/runtime.inc
