...
Completed in `cleanup-1786784246197`: grouped non-registered Pokémon stage/type/Rule Box/Pokémon V compatibility sets under `LegacyPokemonClassification` in `core/card_classification.inc`, then made `retreat_cost()` defer to registered `CardDefinition::retreat_cost` before the shrinking legacy fallback. Registered Regidrago V, Mawile-GX, Oricorio, and Appletun therefore use the same exact-print metadata already asserted by their card modules. This changes ownership only; search legality, route strategy, DCI/UDP/AMR, K0/K1, connector ordering, and retreat decisions remain at their prior owners. Registry: https://github.com/FlareZ123/pokemon-sims/blob/main/src/cards/card_registry.hpp Classification owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/card_classification.inc Advanced retreat procedure: https://github.com/FlareZ123/pokemon-sims/blob/main/EN_advanced_manual-2025-transcription-structured.md

Next catalog step: migrate the remaining `LegacyCardCatalog` and `LegacyPokemonClassification` entries one card at a time through the normal ownership workflow. Delete each compatibility row only when that card has an explicit `CardDefinition`, registration, exact-print source, and focused metadata test. Keep gameplay resolution and strategic policy at their current owners during metadata-only migrations.
...
Next composition step: inventory the remaining named composition stages for a forwarding-only seam whose exact textual boundary is covered by source contracts. Keep gameplay route decisions in their current policy owners and keep each alias setup attached to the include/teardown sequence it protects.
