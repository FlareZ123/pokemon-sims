...
Compatibility bridges are deliberately behavior-free. A bridge may carry only include guards, ownership/source comments, and the canonical include. Before retirement, prove every live consumer and raw-source/tooling consumer has moved to the canonical path, preserve the identical textual boundary, then delete the bridge in a separate mechanical commit. Current bridges covered by this rule include `src/trace_engine_v2/part_issue_1516_quick_ball_tapu_crispin_override.inc` and `src/trace_engine_v2/core/card_context_adapter.hpp`.
