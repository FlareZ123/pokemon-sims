...
- `part_001.inc` keeps the payload predicate as an explicitly non-executable block comment for raw-source inspection, replacing the preprocessor-disabled mirror while preserving a single executable implementation. `part_000.inc` keeps its legacy line-anchor range and now documents that role directly. C++ textual-include semantics: https://eel.is/c++draft/cpp.include
- Future retirement of either shim requires migrating every raw-source reader and same-repository line anchor first. Validate strict compilation, full regression and sanitizer suites, permanent `--simulate-this` audits, and the paired T2/T3 matrix before merge.
