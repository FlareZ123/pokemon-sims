...
- `src/trace_engine_v2/core/tate/package.inc` owns the established discard-provenance, Tate attachment, and Tate action override order. `core/tate/discard_recovery_provenance.inc`, `core/tate/attachment_policy.inc`, and `core/tate/action_policy.inc` are the live named lower-level owners; the historical root Tate paths are compatibility/source-contract seams while references migrate: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/tate/package.inc
- `src/trace_engine_v2/core/routes/tapu_tate_switch_policy.inc` owns the complete Tapu/Tate switch route body. The historical `src/trace_engine_v2/part_tapu_tate_switch_override.inc` path is a compatibility include while source-contract references migrate. Canonical owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/tapu_tate_switch_policy.inc
...
The Tate package now composes discard/recovery provenance, proactive attachment, and the Tate action body from `core/tate/discard_recovery_provenance.inc`, `core/tate/attachment_policy.inc`, and `core/tate/action_policy.inc` at the same member positions. Historical root paths remain compatibility/source-contract seams while references migrate. Canonical package: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/tate/package.inc C++ textual-include semantics: https://eel.is/c++draft/cpp.include

The complete Tapu/Tate switch route body now has its canonical organized owner at `core/routes/tapu_tate_switch_policy.inc`. The historical `part_tapu_tate_switch_override.inc` path is a thin compatibility include while source-contract references migrate. Canonical owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/tapu_tate_switch_policy.inc
...
- Tate/provenance composition: `src/trace_engine_v2/core/tate/package.inc`; discard/recovery provenance, proactive attachment, and Tate action now compose from named lower-level owners under `core/tate/`.
- Tapu/Tate switch route policy: `src/trace_engine_v2/core/routes/tapu_tate_switch_policy.inc`.
...
8. Keep `core/tate/package.inc` as the sole Tate/provenance composition include. The three Tate member bodies now have named owners under `core/tate/`; migrate source-contract anchors to the canonical paths before retiring the historical mirrors. Keep `core/routes/tapu_tate_switch_policy.inc` as the canonical Tapu/Tate switch body and migrate its historical root references before deleting the compatibility include.
