...
- `src/trace_engine_v2/core/regidrago_line_helpers.inc` and `src/trace_engine_v2/core/steven_package_policy.inc` are retired after repository-wide reference checks found no live include or symbol consumer. The canonical composition spines already own Steven ordering, and active board/evolution policy no longer depends on the one-function Regidrago shim.
- Future mechanical cleanup should treat zero-reference single-function and include-only `.inc` shims as deletion candidates only after current strict compilation, canonical composition inspection, and repository-wide reference checks establish that no declaration-order or source-anchor dependency remains.
