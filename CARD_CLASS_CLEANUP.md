...
`src/trace_engine_v2/core/routes/steven_package_policy.inc` now owns all four source-bounded Steven continuations through colocated route files: `steven_latias_t3_policy.inc` (#1745), `steven_t4_package_policy.inc` (#1771), `steven_t3_package_policy.inc` (#1772), and `steven_latias_blender_policy.inc` (#2622). The historical root implementation files were retired in behavior-neutral switching commits while preserving the package's established textual order and every existing direct card/rule citation. Package owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/steven_package_policy.inc Route specifications: https://github.com/FlareZ123/pokemon-sims/issues/1745 https://github.com/FlareZ123/pokemon-sims/issues/1771 https://github.com/FlareZ123/pokemon-sims/issues/1772 https://github.com/FlareZ123/pokemon-sims/issues/2622

Next mechanical Steven step: audit repeated read-only route projections inside this package for a reusable `core/routes/` helper only where the inputs and ordering semantics are identical. Keep issue-specific route admission, K0/K1, DCI/JIT, lock projection, and action priority in their existing policy functions; any gameplay change still requires the separate confirmed-bug workflow. C++ textual-include semantics: https://eel.is/c++draft/cpp.include Policy boundary: https://github.com/FlareZ123/pokemon-sims/blob/main/docs/POLICY_DECISIONS.md
