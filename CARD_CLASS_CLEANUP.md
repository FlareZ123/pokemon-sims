...
The #3222 Steven / Brilliant Blender semantic fallback now has a canonical route owner at `src/trace_engine_v2/core/routes/steven_blender_semantic_policy.inc`. The historical root `part_issue_3222_steven_blender_semantic_override.inc` is reduced to a compatibility include so existing textual consumers retain the identical member boundary while route policy lives under `core/routes/`. The completion predicate evaluates the missing Grass/Fire counts once and names the two held-energy completion states without changing admission. Steven's Resolve: https://api.pokemontcg.io/v2/cards/sm7-145 Brilliant Blender: https://api.pokemontcg.io/v2/cards/sv8-164 Regidrago VSTAR: https://api.pokemontcg.io/v2/cards/swsh12-136 Canonical owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/steven_blender_semantic_policy.inc

Next Steven cleanup step: migrate duplicated Gladion Prize-exchange mutations to `resolve_gladion_prize_exchange()` only when reveal and target-selection semantics match exactly. Continue retiring composition-only `part_*steven*` forwarders whose canonical `core/routes/` owner can replace them at the identical textual boundary. Retire the #3222 root compatibility include only after repository-wide raw-source and tooling references are proven absent. Keep the new `composition/steven_blender_overrides.inc` boundary stable until those inner route seams can be retired without changing macro lifetime.
