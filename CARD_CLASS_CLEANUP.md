...
- `src/trace_engine_v2/composition/engine_body.inc` now composes catalog and classification ownership directly. The historical `part_000.inc` and `part_001.inc` bootstrap/source-contract shims are retired, and `tests/generate_unified_tests.py` derives external headers from the test sources it actually composes instead of reading a retired bootstrap fragment. Canonical composition owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/composition/engine_body.inc
- `src/trace_engine_v2/core/locks/garbodor_policy.inc` is the sole Garbodor scenario-timing and Ability-lock policy owner. `GarbodorScenarioPolicy` and `GarbodorAbilityLockPolicy` are explicit final stateless classes; `GarbodorAbilityLockPolicy::LockState` names the four modeled lock facts, and Engine centralizes those facts through `garbodor_lock_state_for()` before pure policy composition. `composition/engine_body.inc` and `src/regidrago_sim.cpp` reference this canonical organized path directly, and the historical `src/trace_engine_v2/core/garbodor_lock_policy.inc` compatibility include is retired. Garbodor / Garbotoxin: https://api.pokemontcg.io/v2/cards/xy9-57 Boost Shake: https://api.pokemontcg.io/v2/cards/swsh7-142 Path to the Peak: https://api.pokemontcg.io/v2/cards/swsh6-148 Canonical owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/locks/garbodor_policy.inc
- `src/trace_engine_v2/core/payload_hand_policy.inc` owns shared Dragon-payload zone and preference queries. `PayloadZonePolicy` and `PayloadPreferencePolicy` are explicit final stateless policy classes so traversal and preference order stay centralized without becoming extensible Engine subtypes. Both physical-zone traversal and preference traversal now accept generic Card ranges, allowing fixed-size route plans to reuse the same policy without temporary vectors.
- `src/trace_engine_v2/core/board_state_policy.inc` owns reusable board traversal and board-index queries. Recovery Supporter logic now reuses its canonical open Pokémon V Tool-slot and evolvable Regidrago V presence queries instead of duplicating Active/Bench scans. Canonical owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/board_state_policy.inc
...
- `src/trace_engine_v2/core/tapu_connector_policy.inc` owns the copy-aware Tapu species-presence projection through final stateless `TapuConnectorPolicy`; Engine supplies current board presence plus legally known hand/deck copy information. Wonder Tag remains a per-copy Ability from a Tapu Lele-GX played from hand onto the Bench: https://api.pokemontcg.io/v2/cards/sm2-60 Knowledge-state policy: https://github.com/FlareZ123/pokemon-sims/blob/main/docs/POLICY_DECISIONS.md#knowledge-states
...
- `src/trace_engine_v2/core/tate/package.inc` owns the established discard-provenance, Tate attachment, and Tate action override order. `core/tate/discard_recovery_provenance.inc`, `core/tate/attachment_policy.inc`, and `core/tate/action_policy.inc` are the live named owners. The historical Tate action root fragment is retired; only the older provenance and attachment source-contract mirrors remain while their references migrate: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/tate/package.inc
...
- `src/trace_engine_v2/core/mysterious_treasure_target_policy.inc` owns the established strategic Mysterious Treasure target priority through `MysteriousTreasureTargetPolicy`. Its `TargetOrder` names the immutable priority shape and `preferred_targets()` exposes that single stored order by const reference. That route priority is intentionally separate from `SearchConnectorFallbackPolicy`, whose target order applies only after a legal K1 search has started. The remaining `part_009a.inc` route delegates pure physical payload-zone membership to `PayloadZonePolicy`, while DCI/JIT admission, K0/K1 target visibility, connector domination, and action execution remain Engine-owned. Mysterious Treasure: https://api.pokemontcg.io/v2/cards/sm6-113 Payload owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/payload_hand_policy.inc
- `src/trace_engine_v2/core/routes/active_vstar_steven_route_policy.inc` co-locates the active-VSTAR Steven/Crispin/Treasure route with `ActiveVstarStevenProjectionPolicy`. Its `KnownDeckSearchState` now owns the pure Grass/Fire/Treasure-target search-count bundle while the policy retains next-draw risk arithmetic. Engine still owns K1 visibility, route admission, lock checks, DCI/JIT timing, card availability, and all physical actions. Steven's Resolve: https://api.pokemontcg.io/v2/cards/sm7-145 Crispin: https://api.pokemontcg.io/v2/cards/sv7-133 Route-risk provenance: https://github.com/FlareZ123/pokemon-sims/issues/1098
- `src/trace_engine_v2/core/routes/crispin_supported_route_policy.inc` keeps held-Crispin route admission and state reads in Engine while named projection helpers centralize the repeated one-found and two-found Apex-energy checks. Crispin: https://api.pokemontcg.io/v2/cards/sv7-133 Regidrago VSTAR / Apex Dragon: https://api.pokemontcg.io/v2/cards/swsh12-136
- `src/trace_engine_v2/core/routes/professor_burnet_ready_turn_policy.inc` remains the Professor Burnet route-admission and action owner. `core/routes/professor_burnet/selection_policy.inc` owns the fixed two-card selection limit plus the exact payload and no-discard-control thinning priority constants through final stateless `ProfessorBurnetSelectionPolicy`; the route reuses `core/board_state_policy.inc` for Bench presence. Engine retains K0/K1 visibility, DCI/JIT admission, Supporter contention, Energy projections, and physical discard/search execution. Professor Burnet: https://api.pokemontcg.io/v2/cards/swsh12tg-TG26 Regidrago VSTAR / Apex Dragon: https://api.pokemontcg.io/v2/cards/swsh12-136 Canonical board owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/board_state_policy.inc
- `src/trace_engine_v2/core/forretress/scenario_extension.inc` owns physical scenario-range append and label traversal through final stateless `ScenarioRangePolicy`. `ScenarioExtension` owns each extension range plus extension-first fallback lookup, while the concrete Garbodor / Boost Shake scenario family stays beside those mechanics. Canonical owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/forretress/scenario_extension.inc
...
The namespace-scope Forretress composition has its canonical package at `src/trace_engine_v2/core/forretress/package.inc`. Public scenario aliases and registry declarations live directly at that package boundary before `src/trace_engine_v2/core/forretress/scenario_extension.inc`. `ScenarioRangePolicy` now centralizes physical range append and label traversal, and `ScenarioExtension` centralizes extension-first fallback lookup while retaining range ownership. The former `scenario_contract.inc` and `garbodor_scenario_extension.inc` fragments remain retired. The historical `src/trace_engine_v2/part_forretress_ex_combo.inc` path remains a thin compatibility include while the final `part_014c.inc` source-boundary consumer is live. Canonical package: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/forretress/package.inc Garbodor / Garbotoxin: https://api.pokemontcg.io/v2/cards/xy9-57 Boost Shake: https://api.pokemontcg.io/v2/cards/swsh7-142 C++ textual-include semantics: https://eel.is/c++draft/cpp.include
...
The Hisuian Heavy Ball route body remains owned by `src/trace_engine_v2/core/routes/hisuian_heavy_ball_policy.inc`. `src/trace_engine_v2/composition/opening_engine_overrides.inc` now composes that complete member body directly at the established Engine boundary before the shared search-connector helpers. The historical `src/trace_engine_v2/part_008b.inc` forwarding wrapper is retired and should not be recreated. K0/K1 inspection timing, Prize recovery preference, Supporter-contention projections, and trace behavior remain in the same strategy layer. Hisuian Heavy Ball: https://api.pokemontcg.io/v2/cards/swsh10-146 C++ textual-include semantics: https://eel.is/c++draft/cpp.include

The complete search-connector preparation and post-search fallback helpers have one named owner at `src/trace_engine_v2/core/routes/search_connector_helpers.inc`. `SearchConnectorFallbackPolicy` centralizes the printed legal fallback orders and shared first-available K1 traversal for Mysterious Treasure, Quick Ball, and Ultra Ball. Mysterious Treasure's separate strategic priority remains owned by `core/mysterious_treasure_target_policy.inc`. `src/trace_engine_v2/composition/opening_engine_overrides.inc` includes this canonical owner directly after the Heavy Ball policy, and the retired `part_008b.inc` wrapper no longer participates in composition. C++ textual-include semantics: https://eel.is/c++draft/cpp.include Mysterious Treasure: https://api.pokemontcg.io/v2/cards/sm6-113 Quick Ball: https://api.pokemontcg.io/v2/cards/swsh1-179 Ultra Ball: https://api.pokemontcg.io/v2/cards/swsh12pt5-146

The Tate package now composes discard/recovery provenance, proactive attachment, and Tate action policy from `core/tate/discard_recovery_provenance.inc`, `core/tate/attachment_policy.inc`, and `core/tate/action_policy.inc` at the same member positions. The historical root Tate action fragment is retired. The two older provenance and attachment root bodies remain only as source-contract mirrors while their references migrate. Canonical package: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/tate/package.inc C++ textual-include semantics: https://eel.is/c++draft/cpp.include
...
`PayloadZonePolicy` now accepts any Card range with `begin()` / `end()` semantics, so fixed-size route cost plans can reuse the same physical membership and count traversal without temporary vectors or duplicated STL scans. `PayloadPreferencePolicy::first_preferred_in_zone` follows the same generic Card-range contract, preserving the established strategic order while allowing future fixed-size route plans to reuse it directly. The issue-1673 Secret Box deadline cost plan is the first `std::array<Card, 3>` consumer of the generic zone seam. Canonical route: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/issue_1673_secret_box_payload_deadline_policy.inc Secret Box: https://api.pokemontcg.io/v2/cards/sv6-163
...
- Garbodor scenario and Ability-lock composition: `src/trace_engine_v2/core/locks/garbodor_policy.inc`; `GarbodorAbilityLockPolicy::LockState` owns the pure input shape, `garbodor_lock_state_for()` centralizes Engine fact collection, and the retired `src/trace_engine_v2/core/garbodor_lock_policy.inc` path must not be recreated. Garbodor: https://api.pokemontcg.io/v2/cards/xy9-57 Path to the Peak: https://api.pokemontcg.io/v2/cards/swsh6-148
- Setup lifecycle: `src/trace_engine_v2/core/setup_lifecycle.inc`.
- Recovery Supporter policy: `src/trace_engine_v2/core/recovery_supporter_policy.inc`; recovery priority, future-turn axis projections, and Supporter action sequencing stay here, while pure Active/Bench presence queries reuse `core/board_state_policy.inc`.
...
- Tapu Lele-GX copy-aware connector projection: `src/trace_engine_v2/core/tapu_connector_policy.inc` via `TapuConnectorPolicy`; keep board presence and K0/K1 copy knowledge in Engine. Tapu Lele-GX / Wonder Tag: https://api.pokemontcg.io/v2/cards/sm2-60
...
- Tate/provenance composition: `src/trace_engine_v2/core/tate/package.inc`; discard/recovery provenance, proactive attachment, and Tate action policy now compose from named owners under `core/tate/`. The action root fragment is retired; source-contract cleanup remains for the two older mirrors.
...
- Mysterious Treasure strategic target priority: `src/trace_engine_v2/core/mysterious_treasure_target_policy.inc` via `MysteriousTreasureTargetPolicy` and its immutable `TargetOrder`.
- Active-VSTAR Steven next-draw and known-search projection: `src/trace_engine_v2/core/routes/active_vstar_steven_route_policy.inc` via co-located `ActiveVstarStevenProjectionPolicy` and `KnownDeckSearchState`.
- Held-Crispin Apex energy projection: `src/trace_engine_v2/core/routes/crispin_supported_route_policy.inc` via its named projection helpers; route admission and state reads remain Engine-owned.
- Professor Burnet fixed selection limit and priority constants: `src/trace_engine_v2/core/routes/professor_burnet/selection_policy.inc` via `ProfessorBurnetSelectionPolicy`; Burnet route admission and action execution remain in `professor_burnet_ready_turn_policy.inc`.
- Forretress namespace registry contract: `src/trace_engine_v2/core/forretress/package.inc`; `src/trace_engine_v2/core/forretress/scenario_extension.inc` owns `ScenarioRangePolicy`, range-owning `ScenarioExtension`, extension-first fallback lookup, and the concrete Garbodor / Boost Shake extension family; runtime mechanics remain in `core/forretress/runtime.inc`.
...
6. Continue replacing route-local and recovery-local board traversals with `core/board_state_policy.inc` only when first-match/order semantics are identical. The Quick Ball/Latias completion route, Professor Burnet Bench-presence queries, Lusamine Forest Seal Stone holder query, and Roseanne evolvable-Regidrago query are migrated; preserve route-local JIT, recovery priority, Supporter contention, evolution decisions, connector selection, and action execution.
7. Keep Forretress reusable scenario and runtime ownership under `core/forretress/`, with `core/forretress/package.inc` as both the namespace-scope composition owner and registry-declaration boundary, and `core/forretress/scenario_extension.inc` as the single scenario-extension owner. Physical range append/lookup and extension-first fallback lookup are now centralized in that owner. The former scenario-contract and Garbodor split fragments remain retired. Migrate the remaining `part_014c.inc` consumer to the canonical package before deleting `part_forretress_ex_combo.inc`.
8. Keep `core/tate/package.inc` as the sole Tate/provenance composition include. The action body has moved to `core/tate/action_policy.inc` and the historical root action fragment is retired. Migrate source-contract anchors for the remaining provenance and attachment mirrors, then retire those mirrors when repository-wide references are gone.
9. Prefer named pure-projection members over route-local anonymous projections when a projection is reused or has a distinct policy contract. `ActiveVstarStevenProjectionPolicy::KnownDeckSearchState` and the held-Crispin energy helpers are current examples; keep admission and state reads in Engine.
...
13. Reuse generic `PayloadZonePolicy` and `PayloadPreferencePolicy` range operations for fixed-size Card cost plans and physical route-zone queries only when the query is purely membership, count, first-match traversal, or established payload-preference traversal; keep route-specific DCI/JIT admission outside the utility classes.
...
15. Keep `MysteriousTreasureTargetPolicy` limited to strategic route priority and `SearchConnectorFallbackPolicy` limited to legal post-search K1 fallback ordering/traversal. Keep the former policy's `TargetOrder` as the single immutable priority representation. Continue moving the remaining Mysterious Treasure member body out of `part_009a.inc` only when the full continuation can migrate atomically without changing declaration order. C++ textual-include semantics: https://eel.is/c++draft/cpp.include Mysterious Treasure: https://api.pokemontcg.io/v2/cards/sm6-113
16. Keep `ActiveVstarStevenProjectionPolicy` co-located with `active_vstar_steven_route_policy.inc`; the separate projection fragment is retired and should not be recreated. Future cleanup must leave K1 visibility, action-specific locks, DCI/JIT, card availability, route admission, and physical execution with the route owner. Steven's Resolve: https://api.pokemontcg.io/v2/cards/sm7-145 Crispin: https://api.pokemontcg.io/v2/cards/sv7-133
17. Keep `composition/opening_engine_overrides.inc` pointed directly at `core/routes/hisuian_heavy_ball_policy.inc` and `core/routes/search_connector_helpers.inc`; `part_008b.inc` is retired and should not be recreated. Retire similar numbered forwarding shims only after their macro lifetime and member-boundary contracts are explicitly preserved. C++ textual-include semantics: https://eel.is/c++draft/cpp.include
18. Encapsulate speculative projection markers in explicit state holders once their ownership boundary is clear. Keep physical zone changes and DCI/JIT decisions in Engine; `core/legacy_star_projection_provenance.inc` is the reference seam for this cleanup direction. Regidrago VSTAR / Legacy Star: https://api.pokemontcg.io/v2/cards/swsh12-136 Pokémon Communication: https://api.pokemontcg.io/v2/cards/sm9-152
19. Keep `ProfessorBurnetSelectionPolicy` limited to the route's fixed selection limit and established payload/dead-thinning order. Do not move Burnet legality, hidden-zone visibility, strict-JIT timing, Supporter contention, Energy completion, or physical deck/discard transitions out of the Engine route owner. Professor Burnet: https://api.pokemontcg.io/v2/cards/swsh12tg-TG26
20. Keep `core/locks/garbodor_policy.inc` as the sole Garbodor lock owner. The historical `core/garbodor_lock_policy.inc` compatibility include is retired and should not be recreated. Preserve `LockState` as a pure value shape and keep live scenario/state reads in the Engine snapshot helper; future lock cleanup must preserve independent Garbotoxin and Rule Box lock axes. Garbodor / Garbotoxin: https://api.pokemontcg.io/v2/cards/xy9-57 Path to the Peak: https://api.pokemontcg.io/v2/cards/swsh6-148 Scenario contract: https://github.com/FlareZ123/pokemon-sims/issues/2808
21. Keep Tapu copy-awareness split at the current boundary: `TapuConnectorPolicy` owns only the pure species-presence predicate, while Engine owns board state and legally known copy counts. Move the surrounding legacy Tapu alias family only as an atomic composition unit once its macro lifetime can be preserved. Tapu Lele-GX / Wonder Tag: https://api.pokemontcg.io/v2/cards/sm2-60 C++ textual-include semantics: https://eel.is/c++draft/cpp.include
