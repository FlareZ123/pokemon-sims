...
`src/trace_engine_v2/core/payload_hand_policy.inc` owns payload preference order, payload-zone membership, and payload-zone counts through `PayloadPreferencePolicy`. Route files should reuse `payload_zone_contains()` for membership checks instead of open-coding `std::any_of(..., is_payload)`. The issue-1771 Steven continuation now uses this shared seam as the first migrated route. Keep payload role, DCI/UDP, strict-JIT admission, and connector priority in Engine strategy. Regidrago VSTAR / Apex Dragon: https://api.pokemontcg.io/v2/cards/swsh12-136 DCI/JIT policy: https://github.com/FlareZ123/pokemon-sims/blob/main/docs/POLICY_DECISIONS.md#dcijit-treatment Route owner: https://github.com/FlareZ123/pokemon-sims/blob/main/src/trace_engine_v2/core/routes/issue_1771_steven_t4_package_override.inc

Next payload-policy step: migrate repeated raw payload membership scans in other route `.inc` files one package at a time, preserving each route's existing K0/K1 and JIT admission predicates. Do not replace search-specific checks with zone membership where printed searchability or known-deck cardinality is the actual condition. K0/K1 policy: https://github.com/FlareZ123/pokemon-sims/blob/main/docs/POLICY_DECISIONS.md#knowledge-states
