...
4. A payload already in discard remains a completed readiness axis on later turns. Strict and matchup-flex JIT control when policy may first commit a protected payload: only after the current turn has an energy-complete attacker plus a deterministic VSTAR and Active-position route.
