...
8. **Delayed routes last.** Lusamine recovery and draw supporters are used only after direct setup connectors fail. Tate & Liza's draw mode is a dead-hand recovery route; switch mode has priority when it immediately fixes Active position.
...
- `regidrago_policy_tests`: 29 core exact-state rule and policy fixtures.
