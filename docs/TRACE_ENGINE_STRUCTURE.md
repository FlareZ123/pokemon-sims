...
Behavioral refactors in `src/` remain source-bound evidence changes. Refresh the baseline and multi-deck provenance manifests after such edits, then use the normal pull-request CI as the compiler, deterministic-trace, sanitizer, contract, and paired-matrix merge gate.
