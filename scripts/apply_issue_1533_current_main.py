...
if "complete_issue_1533_blender_treasure_latias_route" in source:
    raise SystemExit("issue 1533 production function already exists")
if source.count(function_anchor) != 1:
    raise SystemExit(f"issue 1533 function anchor count: {source.count(function_anchor)}")
source = source.replace(function_anchor, function_text + function_anchor, 1)

call_anchor = "    play_chaotic_swell();\n    play_field_blower();\n"
call_text = dedent(
    r'''
    play_chaotic_swell();
    play_field_blower();
...
if source.count(call_anchor) != 1:
    raise SystemExit(f"issue 1533 call anchor count: {source.count(call_anchor)}")
source = source.replace(call_anchor, call_text, 1)
source_path.write_text(source, encoding="utf-8")

test_path = Path("tests/issue_1533_blender_treasure_latias_tests.cpp")
