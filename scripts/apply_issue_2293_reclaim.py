...
late_helper = r'''  bool complete_issue_2293_late_gladion_latias_promotion() {
    if (scenario_.dci != DciProfile::StrictJit || !scenario_.going_first ||
        state_.turn != 4 || state_.turn_ended || !state_.supporter_used ||
        count_of(state_.discard, Card::Gladion) == 0 ||
    ...
    // A late Wonder Tag -> Gladion line can put the K1-known Prize Latias ex into
    // hand after the ordinary Basic-play fixed point. Playing that Basic to the
    // Bench is still legal during the same turn; Skyliner then removes Oricorio's
    // Retreat Cost and immediately promotes the already-complete Regidrago VSTAR.
    // Gladion: https://api.pokemontcg.io/v2/cards/sm4-95
    // Latias ex / Skyliner: https://api.pokemontcg.io/v2/cards/sv8-76
    // Oricorio: https://api.pokemontcg.io/v2/cards/sm2-55
...
# K1 Wonder Tag must bridge through Gladion to the known prized Latias ex, then
# replay that same-turn Basic Bench action before the final Skyliner promotion.
