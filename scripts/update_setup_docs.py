...
The saved traces are independent audit examples. They are not asserted to use the same seeds or exact first-ready turns as the separately registered CTest deadline regressions in `CMakeLists.txt`: https://github.com/FlareZ123/pokemon-sims/blob/main/CMakeLists.txt#L217-L223
...
CTest validates its registered deadline cases. The baseline generator separately reproduces the six saved audit traces and aggregate matrix from [`../results/baseline_manifest.json`](../results/baseline_manifest.json): https://github.com/FlareZ123/pokemon-sims/blob/main/results/baseline_manifest.json. The generator uses an exclusive lock and atomic file replacement.
