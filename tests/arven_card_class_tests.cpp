...
void test_registry_metadata_and_supporter_classification() {
  ...
  require(definition->kind == sim::cards::CardKind::Trainer,
          "Arven must remain a Trainer."); // Supporter procedure: https://github.com/FlareZ123/pokemon-sims/blob/main/EN_advanced_manual-2025-transcription-structured.md
  require(definition->trainer_kind == sim::cards::TrainerKind::Supporter,
          "Arven must remain a Supporter."); // Exact print: https://api.pokemontcg.io/v2/cards/sv1-166
  require(sim::is_supporter(sim::Card::Arven),
          "Compatibility classification must source Arven as a Supporter.");
  ...
  // This wave is metadata-only; strategic Item + Tool target choice remains in Engine.
  // Cleanup boundary: https://github.com/FlareZ123/pokemon-sims/issues/3577
}
