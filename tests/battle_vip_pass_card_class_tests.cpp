...
void test_registry_metadata_and_item_classification() {
  ...
  require(definition->id == sim::Card::BattleVipPass,
          "Battle VIP Pass registry id must remain stable.");
  require(definition->canonical_id == "swsh8-225",
          "Battle VIP Pass canonical print changed."); // https://api.pokemontcg.io/v2/cards/swsh8-225
  require(definition->name == "Battle VIP Pass", "Battle VIP Pass name changed.");
  require(definition->kind == sim::cards::CardKind::Trainer,
          "Battle VIP Pass must remain a Trainer."); // https://api.pokemontcg.io/v2/cards/swsh8-225
  require(definition->trainer_kind == sim::cards::TrainerKind::Item,
          "Battle VIP Pass must remain an Item.");
  require(sim::is_item(sim::Card::BattleVipPass),
          "Compatibility classification must source Battle VIP Pass as an Item.");
  require(sim::name(sim::Card::BattleVipPass) == "Battle VIP Pass",
