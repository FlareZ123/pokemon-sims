...
sim::Engine make_engine(sim::State state, sim::TraceLog* trace = nullptr) {
  const sim::Scenario scenario{"empty-deck-search", sim::DciProfile::StrictJit,
                               sim::LockMode::None, false, 4};
  const sim::DeckRecipe recipe{sim::baseline_recipe()};
  static std::mt19937_64 rng{322};
...
void test_search_item_does_not_pay_its_cost() {
  ...
  sim::Engine engine = make_engine(std::move(state));
...
void test_star_alchemy_preserves_the_vstar_power() {
  ...
  const sim::State& after = sim::EngineTestAccess::state(engine);
  if (after.vstar_power_used || sim::EngineTestAccess::outcome(engine).used_fss) {
...
void test_optional_entry_search_is_declined_after_legal_bench() {
  ...
  sim::Engine engine = make_engine(std::move(state), &trace);
  ...
  // https://compendium.pokegym.net/category/7-gameplay/searching-deck-or-discard/
  if (!sim::EngineTestAccess::bench_from_hand(engine, sim::Card::Oricorio, true)) {
  ...
  if (trace.lines.empty() || trace.lines.back().find("ABILITY DECLINED") == std::string::npos) {
