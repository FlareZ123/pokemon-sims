...
def compile_and_run(source_name: str, binary_name: str) -> None:
    compiler = os.environ.get("CXX", "c++")
    source = REPO_ROOT / "experiments" / source_name
...
def run_ev_vs_quick_ball_ci_experiment() -> None:
    ...
    compile_and_run("ev_vs_quick_ball.cpp", "ev-vs-quick-ball-experiment")
    compile_and_run("ev_vs_quick_ball_garbodor.cpp", "ev-vs-quick-ball-garbodor-experiment")
    ...
    garbodor = (REPO_ROOT / "ev-vs-quick-ball-garbodor.csv").read_text(encoding="utf-8")
    garbodor_paired = (REPO_ROOT / "ev-vs-quick-ball-garbodor-paired.csv").read_text(encoding="utf-8")
    summary.write_text(
        ...
        "AGGREGATE\n" + aggregate + "\nPAIRED\n" + paired + "\nWITNESSES\n" + witnesses
        + "\nFULL BASIC ABILITY LOCK (GARBODOR)\n" + garbodor
        + "\nGARBODOR PAIRED\n" + garbodor_paired,
        encoding="utf-8",
        newline="",
    )
