...
void test_registry_metadata_and_supporter_classification() {
  ...
  require(definition != nullptr,
          "Grant must be explicitly registered.");
  require(definition->id == sim::Card::Grant,
          "Grant registry id must remain stable.");
  require(definition->canonical_id == "swsh10-144",
          "Grant canonical print changed."); // Card data: https://api.pokemontcg.io/v2/cards/swsh10-144
  require(definition->name == "Grant",
          "Grant name changed.");
  require(definition->kind == sim::cards::CardKind::Trainer,
          "Grant must remain a Trainer."); // Trainer procedure: https://github.com/FlareZ123/pokemon-sims/blob/main/EN_advanced_manual-2025-transcription-structured.md
  require(definition->trainer_kind == sim::cards::TrainerKind::Supporter,
          "Grant must remain a Supporter."); // Exact print and one-Supporter reminder: https://api.pokemontcg.io/v2/cards/swsh10-144
  require(definition->source_url == "https://api.pokemontcg.io/v2/cards/swsh10-144",
          "Grant source URL changed.");
  ...
  require(!sim::is_item(sim::Card::Grant),
          "Grant must not be classified as an Item.");
