...
void test_registry_metadata_and_supporter_classification() {
  ...
  require(definition != nullptr, "Guzma must be explicitly registered.");
  require(definition->id == sim::Card::Guzma,
          "Guzma registry id must remain stable.");
  require(definition->canonical_id == "sm3-115",
          "Guzma canonical print changed.");  // Exact card data: https://api.pokemontcg.io/v2/cards/sm3-115
  require(definition->name == "Guzma", "Guzma name changed.");
  require(definition->kind == sim::cards::CardKind::Trainer,
          "Guzma must remain a Trainer.");  // Trainer identity: https://api.pokemontcg.io/v2/cards/sm3-115
  require(definition->trainer_kind == sim::cards::TrainerKind::Supporter,
          "Guzma must remain a Supporter.");  // Printed subtype: https://api.pokemontcg.io/v2/cards/sm3-115 ; procedure: https://github.com/FlareZ123/pokemon-sims/blob/main/EN_advanced_manual-2025-transcription-structured.md
  require(sim::is_supporter(sim::Card::Guzma),
          "Compatibility classification must source Guzma as a Supporter.");
  require(!sim::is_item(sim::Card::Guzma),
          "Guzma must not be classified as an Item.");  // Printed Trainer subtype: https://api.pokemontcg.io/v2/cards/sm3-115
