...
void test_burnet_preserves_serena_and_held_payload_without_item_outlet() {
  const sim::Scenario scenario{"issue-2408-burnet-over-serena",
                               sim::DciProfile::StrictJit,
                               sim::LockMode::None, false, 4};
  ...
  // Both Supporters establish the sole strict-JIT payload axis this turn. At K1,
  // Burnet can discard a known deck-resident Dragon while preserving Serena and
  // the held Mega Dragonite as discrete later-game resources. Issue #2408 refines
  // the older #1341 Serena subcase only when no live Item payload outlet exists:
  ...
  // Confirmed refinement: https://github.com/FlareZ123/pokemon-sims/issues/2408
  // Validation PR: https://github.com/FlareZ123/pokemon-sims/pull/2871
  sim::EngineTestAccess::choose_supporter(engine);
  const sim::State& result = sim::EngineTestAccess::state(engine);
...
int main() {
  try {
    ...
    test_burnet_preserves_serena_and_held_payload_without_item_outlet();
