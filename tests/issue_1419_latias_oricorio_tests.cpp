...
void test_selects_latias_only_for_complete_oricorio_route() {
  Fixture fixture;
  ...
  // Quick Ball can spend Latias only because Oricorio finds both missing Grass,
  // T3 and T4 provide the two manual attachments, and Blender retains a payload:
  ...
  expect(sim::EngineTestAccess::quick_ball_cost(fixture.engine) ==
             sim::Card::LatiasEx,
         "complete Active-VSTAR Oricorio route did not expose inert Latias ex");
  expect(sim::EngineTestAccess::play_quick_ball(fixture.engine),
         "Quick Ball did not resolve the proven Oricorio search");
  ...
  expect(std::find(after.discard.begin(), after.discard.end(),
         ...
         "Latias ex did not pay Quick Ball's printed discard cost");
...
void test_preserves_latias_at_every_route_boundary() {
...
void test_ordinary_lower_dci_cost_stays_ahead_of_latias() {
  Fixture fixture;
  sim::State state = route_state();
...
void test_source_bound_seed_129_reaches_turn_four() {
  ...
  // Source-bound regression and required legal sequence:
  // https://github.com/FlareZ123/pokemon-sims/issues/1419
  // https://api.pokemontcg.io/v2/cards/swsh1-179
  // https://api.pokemontcg.io/v2/cards/sm2-55
  // https://api.pokemontcg.io/v2/cards/sv8-164
  expect(outcome.first_ready_turn == 4,
         "seed 129 did not reach strict-JIT readiness on turn four");
  expect(trace_contains(trace, "Latias ex (Quick Ball cost)"),
         "seed 129 did not spend dynamically inert Latias ex");
  expect(trace_contains(trace, "Searched a Basic Pokemon: Oricorio") ||
             trace_contains(trace, "Searched a Basic Pokémon: Oricorio"),
         "seed 129 did not search Oricorio");
  expect(trace_contains(trace, "Vital Dance"),
         "seed 129 did not resolve Vital Dance");
}

}  // namespace

int main() {
  test_selects_latias_only_for_complete_oricorio_route();
  test_preserves_latias_at_every_route_boundary();
  test_ordinary_lower_dci_cost_stays_ahead_of_latias();
  test_source_bound_seed_129_reaches_turn_four();
  return 0;
}
