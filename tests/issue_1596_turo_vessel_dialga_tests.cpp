...
const sim::DeckRecipe& registered_pineco_recipe() {
  ...
  if (deck == nullptr) {
    throw std::runtime_error(
        "The registered regidrago-pineco recipe is unavailable.");
  }
  return deck->recipe;
}
...
void test_seed_26_plays_turo_before_vessel() {
  const SeedResult result = run_seed(
      "regidrago-pineco", "matchup-flex-jit/go-second", 26);
  ...
  // Confirmed route bug: https://github.com/FlareZ123/pokemon-sims/issues/1596
...
void test_both_k1_provenances_and_k0_boundary() {
  ...
  // Official Prize, Supporter, return, promotion, Item, discard, search, attachment, and turn procedure: https://www.pokemon.com/static-assets/content-assets/cms2/pdf/trading-card-game/rulebook/par_rulebook_en.pdf
  // Confirmed provenance bug: https://github.com/FlareZ123/pokemon-sims/issues/2000
  std::mt19937_64 deck_rng{200001};
  sim::Engine deck_k1(scenario, recipe, deck_rng);
  sim::EngineTestAccess::set_state(deck_k1, exact_t3_state(), true, false);
  expect(sim::EngineTestAccess::complete_route(deck_k1),
         "The deck-search K1 Turo-Vessel route was rejected.");

  std::mt19937_64 prize_rng{200002};
  ...
  std::mt19937_64 k0_rng{200003};
  sim::Engine k0(scenario, recipe, k0_rng);
  sim::EngineTestAccess::set_state(k0, exact_t3_state(), false, false);
  expect(!sim::EngineTestAccess::complete_route(k0),
         "The Turo-Vessel route used deck identities before K1.");
}
...
void test_shell_seed_43_keeps_existing_t2() {
  const SeedResult result = run_seed(
      "regidrago-shell", "strict-jit/go-first", 43);
  expect(result.outcome.first_ready_turn == 2 &&
             !result.outcome.setup_failed,
