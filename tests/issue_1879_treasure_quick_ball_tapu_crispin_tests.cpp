...
void complete_two_connector_costs_and_energy_route() {
  ...
  // Mysterious Treasure and Quick Ball each require their own discard cost.
  // The first spends route-replaced Earthen Vessel and searches a Dragon; the
  // second spends that Dragon and searches Tapu Lele-GX. Wonder Tag finds
  // Crispin, whose attachment plus the unused manual attachment completes GGF:
...
void one_missing_energy_does_not_require_manual_attachment() {
  Fixture fixture;
  sim::State state = base_state();
  ...
  expect(after.active->grass >= 2 && after.active->fire >= 1,
         "Crispin did not attach the final Fire Energy");
}

void one_type_crispin_requires_manual_attachment() {
  Fixture available;
  sim::State state = base_state();
  ...
  expect(sim::EngineTestAccess::plan(available.engine).has_value(),
         "The route rejected a legal one-type Crispin to manual attachment line");

  expect(sim::EngineTestAccess::play(available.engine),
         "The one-type Crispin route did not use the manual attachment");
  const sim::State& after = sim::EngineTestAccess::state(available.engine);
  expect(after.active->grass >= 2 && after.active->fire >= 1 &&
             after.manual_energy_used,
         "The one-type Crispin route did not complete GGF legally");

  Fixture spent;
  state.manual_energy_used = true;
  sim::EngineTestAccess::set_state(spent.engine, std::move(state));
  expect(!sim::EngineTestAccess::plan(spent.engine).has_value(),
         "The route invented an attachment after one-type Crispin with the manual action spent");
}

void public_state_and_resource_boundaries_are_enforced() {
  {
    Fixture fixture;
    sim::EngineTestAccess::set_state(fixture.engine, base_state(), false);
    expect(!sim::EngineTestAccess::plan(fixture.engine).has_value(),
           "The route read deck identities at K0");
  }
  {
  ...
  }
  {
    ...
    sim::EngineTestAccess::set_state(fixture.engine, std::move(state));
    expect(!sim::EngineTestAccess::plan(fixture.engine).has_value(),
           "The route invented Mysterious Treasure's first discard cost");
  }
  for (const sim::Card missing : {sim::Card::TapuLeleGX,
    ...
    sim::EngineTestAccess::set_state(fixture.engine, std::move(state));
    expect(!sim::EngineTestAccess::plan(fixture.engine).has_value(),
           "The route invented a missing Tapu or Crispin search target");
  }
  {
    Fixture fixture;
    sim::State state = base_state();
    for (const sim::Card payload : {sim::Card::MegaDragonite,
                                    sim::Card::Dragapult}) {
      state.deck.erase(std::remove(state.deck.begin(), state.deck.end(), payload),
                       state.deck.end());
    }
    sim::EngineTestAccess::set_state(fixture.engine, std::move(state));
    expect(!sim::EngineTestAccess::plan(fixture.engine).has_value(),
           "The route invented a missing Dragon payload target");
  }
  {
    Fixture fixture;
    sim::State state = base_state();
    ...
    sim::EngineTestAccess::set_state(fixture.engine, std::move(state));
    const auto plan = sim::EngineTestAccess::plan(fixture.engine);
    expect(plan.has_value() && (*plan)[1] == sim::Card::Dragapult,
           "The route failed to use the remaining legal Dragon payload");
  }
}
...
void exact_registered_witnesses_reach_the_earliest_turn() {
  {
    ...
    expect(trace_contains(trace, "Mysterious Treasure issue-1879 first cost") &&
           ...
           "Seed 146 did not use the composed Treasure-Quick Ball-Tapu-Crispin route");
  }
  {
    sim::TraceLog trace{ true, {} };
    const sim::TrialOutcome result =
    ...
    expect(trace_contains(trace, "Mysterious Treasure issue-1879 first cost") &&
           ...
           "Seed 163 did not use the composed Treasure-Quick Ball-Tapu-Crispin route");
  }
}
...
int main() {
  try {
    ...
    one_missing_energy_does_not_require_manual_attachment();
    one_type_crispin_requires_manual_attachment();
    public_state_and_resource_boundaries_are_enforced();
