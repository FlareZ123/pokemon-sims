...
void test_item_lock_preserves_the_existing_legality_gate() {
  ...
  const sim::State before = sim::EngineTestAccess::state(fixture.engine);

  // Item lock prevents Heavy Ball from changing any game zone. The helper return
  // is scheduler bookkeeping, so legality is established by state invariance:
  // https://api.pokemontcg.io/v2/cards/swsh10-146
  // https://www.pokemon.com/static-assets/content-assets/cms2/pdf/trading-card-game/rulebook/par_rulebook_en.pdf
  // https://github.com/FlareZ123/pokemon-sims/blob/main/docs/POLICY_DECISIONS.md
  static_cast<void>(sim::EngineTestAccess::play_heavy_ball(fixture.engine));
  const sim::State& after = sim::EngineTestAccess::state(fixture.engine);
  expect(after.hand == before.hand && after.prizes == before.prizes &&
             after.discard == before.discard && after.deck == before.deck &&
