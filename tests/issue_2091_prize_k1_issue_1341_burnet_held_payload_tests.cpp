...
void test_equal_turn_burnet_preserves_serena_and_held_payload() {
  ...
  // Prize inspection is an independent K1 provenance path. Once K1 proves a deck
  // Dragon payload, #2408 makes the same resource comparison as deck-inspection K1:
  // Burnet reaches the same strict-JIT turn while preserving Serena and the held
  // Dragon instead of paying Serena's mandatory discard cost.
  ...
  // Official Supporter, Prize, search, and discard procedure: https://www.pokemon.com/static-assets/content-assets/cms2/pdf/trading-card-game/rulebook/par_rulebook_en.pdf
  ...
  // Prize-K1 provenance contract: https://github.com/FlareZ123/pokemon-sims/issues/2091
  ...
  expect(contains(result.hand, sim::Card::Serena),
         "Prize-K1 equal-turn Burnet must preserve Serena.");
  expect(contains(result.hand, sim::Card::MegaDragonite),
         "Prize-K1 equal-turn Burnet must preserve the held Dragon.");
}
...
int main() {
  try {
    ...
    std::cout << "Issue 1341 Burnet held-payload Prize-K1 tests passed\n";
    return 0;
