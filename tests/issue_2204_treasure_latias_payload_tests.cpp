...
int main() {
  ...
  if (!scenario || !deck) throw std::runtime_error("issue-2204 fixture unavailable");
  ...
  if (outcome.first_ready_turn != 4 || outcome.setup_failed ||
      ...
      !has(trace, "T4 | READY")) {
    throw std::runtime_error("issue-2204 route regression");
  }
  return 0;
}
