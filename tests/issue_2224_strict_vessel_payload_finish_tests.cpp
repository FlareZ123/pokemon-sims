...
void test_strict_and_matchup_flex_parity() {
  std::mt19937_64 rng(2224001);

  for (const sim::DciProfile profile : {sim::DciProfile::StrictJit,
                                        sim::DciProfile::MatchupFlexJit}) {
    sim::Engine engine = make_engine(scenario(profile), rng, route_state());
    ...
    expect(has(engine.state().discard, sim::Card::MegaDragonite) &&
               has(engine.state().discard, sim::Card::EarthenVessel),
           "Vessel did not use the held Dragon as its current-turn payload cost.");
    expect(has(engine.state().hand, sim::Card::Grass),
           "Vessel did not search the missing Basic Grass Energy.");
  }
}

void test_route_boundaries() {
  std::mt19937_64 rng(2224002);

  {
    sim::Engine engine = make_engine(
        scenario(sim::DciProfile::NoDiscardControl), rng, route_state());
    expect(!sim::EngineTestAccess::play_route(engine),
  ...
  {
    sim::Engine engine = make_engine(
        scenario(sim::DciProfile::StrictJit, sim::LockMode::FullItem),
        rng, route_state());
    expect(!sim::EngineTestAccess::play_route(engine),
...
void test_registered_seed_939_reaches_t4() {
  ...
  expect(trace_has(trace, "T4 | GLADION") &&
