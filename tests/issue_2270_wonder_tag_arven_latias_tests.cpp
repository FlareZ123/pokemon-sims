...
struct EngineTestAccess {
  ...
  static bool wonder_tag_arven_payload_route(const Engine& engine) {
    return engine.wonder_tag_arven_held_payload_route_available();
  }
};
}  // namespace sim
...
void test_quick_ball_latias_can_complete_the_missing_active_axis() {
  ...
  // The K1 state has a GGF Benched Regidrago VSTAR, a held Dragon payload, and
  // known Arven -> Quick Ball -> Latias ex access. Quick Ball may discard Goodra,
  // search the Basic Latias ex, and Skyliner then gives Basic Dialga-GX free Retreat.
  ...
  // Core Supporter, Item, discard, search, Bench, Ability, and Retreat procedure:
  // https://www.pokemon.com/static-assets/content-assets/cms2/pdf/trading-card-game/rulebook/par_rulebook_en.pdf
  // K1, DCI, strict-JIT, and earliest-route policy:
  ...
  if (!sim::EngineTestAccess::wonder_tag_arven_payload_route(fixture.engine)) {
...
void test_route_stays_live_after_tapu_occupies_its_bench_slot() {
  ...
  // remaining slot is therefore sufficient for Latias ex; requiring two at this
  // point would make the preflight disagree with its own legal continuation.
...
void test_route_respects_item_and_rule_box_ability_locks() {
  ...
  if (sim::EngineTestAccess::wonder_tag_arven_payload_route(item_engine)) {
  ...
  if (sim::EngineTestAccess::wonder_tag_arven_payload_route(ability_engine)) {
