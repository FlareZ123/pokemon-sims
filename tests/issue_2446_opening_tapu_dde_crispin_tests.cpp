...
void test_pre_regidrago_future_target_is_preserved() {
  Fixture fixture;
  sim::State state;
  state.turn = 1;
  ...
  expect(sim::EngineTestAccess::future_wonder_tag_target(fixture.engine),
         "The DDE refinement suppressed the legacy pre-Regidrago future target.");
}
...
int main() {
  try {
    ...
    test_pre_regidrago_future_target_is_preserved();
