...
void test_crispin_replaced_energy_uses_shared_jit_timing() {
  ...
  // The exact #1093 route has no ordinary low-DCI discard fodder. Crispin replaces
  // one held Grass, while Arven, Forest Seal Stone, VSTAR, Blender, and Latias keep
  // the deterministic next-turn setup intact. Both registered JIT profiles share
  // the same ready-turn payload timing, so both must expose the same replacement.
