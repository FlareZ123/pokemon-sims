...
void test_registry_contains_both_seats() {
  ...
  // Boost Shake establishes Garbodor during the opponent's first turn, so the two
  // player seats need separate aggregate rows with different T1 timing:
...
void test_boost_shake_timing_by_seat() {
  ...
  // Regidrago T1 happens before the opponent's Boost Shake turn when going first.
  // Garbotoxin is therefore active from Regidrago T2. Going second, the opponent's
  // first turn has already established Garbodor, so the lock starts on Regidrago T1:
  // https://api.pokemontcg.io/v2/cards/swsh7-142
  // https://api.pokemontcg.io/v2/cards/xy9-57
  // https://github.com/FlareZ123/pokemon-sims/issues/2808
  expect(!sim::Issue2808EngineTestAccess::garbodor_locked(first_engine),
         "Going-first T1 must remain unlocked.");
  expect(sim::Issue2808EngineTestAccess::pokemon_ability_available(first_engine,
                                                                    sim::Card::Oricorio),
         "Pokemon Abilities must be available on going-first T1.");
  ...
  expect(sim::Issue2808EngineTestAccess::garbodor_locked(second_engine),
         "Going-second T1 must start under Garbotoxin.");
}

void test_field_blower_unlock_is_current_turn_only() {
  ...
  // Field Blower may discard Garbodor's Tool, ending Garbotoxin while that Tool is
  // absent. The scenario assumes the maximum-pressure opponent reattaches a Tool
  ...
  expect(sim::Issue2808EngineTestAccess::play_field_blower(engine),
         "Field Blower must unlock a live Dark Asset route.");
  expect(sim::Issue2808EngineTestAccess::garbodor_unlocked_turn(engine) == 1,
         "The unlock marker must be scoped to the current turn.");
  expect(sim::Issue2808EngineTestAccess::pokemon_ability_available(engine,
                                                                    sim::Card::CrobatV),
         "Pokemon Abilities must become available after Field Blower.");

  sim::Issue2808EngineTestAccess::begin_turn(engine, 2);
  expect(sim::Issue2808EngineTestAccess::garbodor_locked(engine),
         "Garbotoxin must automatically return on the next Regidrago turn.");
}

void test_dark_asset_six_card_boundary_is_live() {
  std::mt19937_64 rng{280804};
  const sim::Scenario scenario{"garbodor-shake-ability-lock/go-second",
                               sim::DciProfile::StrictJit, sim::LockMode::None, false, 5};
  sim::Engine engine = make_engine(scenario, rng);
  sim::State state = minimal_state(1);
  state.hand = {sim::Card::FieldBlower, sim::Card::CrobatV, sim::Card::Grass,
                sim::Card::Fire, sim::Card::RegidragoVstar, sim::Card::Crispin};
  sim::Issue2808EngineTestAccess::set_state(engine, std::move(state));

  // Dark Asset draws until six after Crobat V moves from hand to the Bench. A
  // six-card pre-play hand therefore becomes five and still draws one card:
  // https://api.pokemontcg.io/v2/cards/swsh3-104
  // https://github.com/FlareZ123/pokemon-sims/issues/2808
  expect(sim::Issue2808EngineTestAccess::play_field_blower(engine),
         "Six-card hands must still count Dark Asset as a live unlock route.");
}

void test_only_actual_item_lock_blocks_field_blower() {
  std::mt19937_64 rng1{280805};
  ...
  // Item legality remains controlled by the actual Item-lock rule rather than by
...
void test_arven_searches_field_blower_for_live_unlock() {
  ...
  // Arven may search an Item. When Garbotoxin blocks the held Dark Asset connector,
  // Field Blower becomes a live search target while still consuming the one Supporter
  // action for the turn:
...
void test_forest_seal_stone_remains_usable_under_garbotoxin() {
  ...
  // Star Alchemy is the Pokemon Tool's Ability that an attached Pokemon V may use.
  // The repository's source-bound ruling therefore keeps it outside Pokemon-Ability
  // suppression such as Garbotoxin:
  // https://api.pokemontcg.io/v2/cards/swsh12-156
  // https://compendium.pokegym.net/category/5-trainers/forest-seal-stone/
  // https://api.pokemontcg.io/v2/cards/xy9-57
  // https://github.com/FlareZ123/pokemon-sims/issues/2808
  expect(sim::Issue2808EngineTestAccess::garbodor_locked(engine),
         "The exact state must be under Garbotoxin.");
  expect(sim::Issue2808EngineTestAccess::use_fss(engine),
         "Star Alchemy must remain usable under Garbotoxin.");
}
}  // namespace
...
int main() {
  ...
  test_field_blower_unlock_is_current_turn_only();
  test_dark_asset_six_card_boundary_is_live();
