...
void test_real_blockers_remain() {
  {
    ...
    // Going first on turn one cannot use a Supporter.
    // Steven's Resolve: https://api.pokemontcg.io/v2/cards/sm7-145
    // Official turn procedure: https://www.pokemon.com/us/pokemon-tcg/rules
    expect(!sim::EngineTestAccess::route_available(engine),
           "First-turn going-first illegally admitted Steven");
  }
  for (const int turn : {1, 2, 3}) {
    std::mt19937_64 rng{319705ULL + static_cast<std::uint64_t>(turn)};
    sim::Engine engine = make_engine(sim::DciProfile::StrictJit,
                                     sim::LockMode::TurnTwoItem, false, turn, rng);
    // Mysterious Treasure is an Item. In this modeled scenario Item lock begins on
    // the player's second turn and remains active afterward, so any bank whose
    // finish is T2 or later is invalid before Steven consumes the Supporter action.
    // Mysterious Treasure: https://api.pokemontcg.io/v2/cards/sm6-113
    ...
    expect(!sim::EngineTestAccess::route_available(engine),
           "Persistent turn-two Item lock illegally admitted the Treasure route");
  }
  {
    std::mt19937_64 rng{319709};
    sim::Engine engine = make_engine(sim::DciProfile::StrictJit,
                                     sim::LockMode::FullSupporter, false, 1, rng);
    // Steven and Crispin are Supporters: https://api.pokemontcg.io/v2/cards/sm7-145 https://api.pokemontcg.io/v2/cards/sv7-133
    expect(!sim::EngineTestAccess::route_available(engine),
           "Supporter lock illegally admitted the Steven route");
  }
  {
    std::mt19937_64 rng{319710};
