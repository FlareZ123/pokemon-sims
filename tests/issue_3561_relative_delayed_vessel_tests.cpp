...
struct EngineTestAccess {
  ...
  static std::optional<int> delayed_vessel_ready_turn(const Engine& engine) {
    return engine.state_.issue_1447_vessel_ready_turn;
  }
...
sim::State ready_state(const int turn) {
  ...
  state.issue_1447_vessel_ready_turn = turn;
  return state;
}
...
void expect_relative_hold(const int source_turn, const int max_turn) {
  ...
  // #1447 compares this turn with the immediately following turn. Earthen Vessel
  // and Crispin impose no absolute T2/T3 restriction, so T2->T3 and the equivalent
  // T3->T4 state must record the same +1 continuation:
...
void expect_recorded_continuation_plays(const int source_turn, const int max_turn) {
  ...
  expect(sim::EngineTestAccess::delayed_vessel_ready_turn(engine) == source_turn + 1,
         "The source state did not record its immediate continuation.");

  const int ready_turn = source_turn + 1;
  sim::EngineTestAccess::set_state(engine, ready_state(ready_turn));

  // The recorded immediately-following turn must resolve the held Vessel. Its
  // discard cost supplies the Dragon payload while the searched Basic Energy
  // completes Apex Dragon's GGF cost:
  // Earthen Vessel: https://api.pokemontcg.io/v2/cards/sv4-163
  // Regidrago VSTAR / Apex Dragon: https://api.pokemontcg.io/v2/cards/swsh12-136
