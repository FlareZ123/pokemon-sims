...
void test_recorded_t4_continuation_plays_vessel() {
  const sim::Scenario scenario{"issue-3561-relative-finish", sim::DciProfile::StrictJit,
                               sim::LockMode::None, true, 4};
  std::mt19937_64 rng(356104);
  sim::Engine engine(scenario, fixture_recipe(), rng);
  sim::EngineTestAccess::set_state(engine, hold_state(3));
  sim::EngineTestAccess::set_deck_seen(engine, true);
  expect(!sim::EngineTestAccess::run_search_items_one_step(engine),
         "The T3 source state did not hold Vessel.");

  sim::State ready;
  ready.turn = 4;
  ready.active = sim::Pokemon{sim::Card::RegidragoVstar, 2, 1, 1,
                              sim::Tool::None, 0};
  ready.hand = {sim::Card::EarthenVessel, sim::Card::DialgaGX};
  ready.deck = {sim::Card::Grass};
  sim::EngineTestAccess::set_state(engine, std::move(ready));

  // The held Vessel is the same-turn Dragon discard outlet and searches the final
  // Basic Energy on the recorded next turn. Its printed cost/search and Apex's GGF
  // cost are the legality boundary for this continuation:
  // Earthen Vessel: https://api.pokemontcg.io/v2/cards/sv4-163
  // Regidrago VSTAR / Apex Dragon: https://api.pokemontcg.io/v2/cards/swsh12-136
  ...
  // Relative-turn correction: https://github.com/FlareZ123/pokemon-sims/issues/3561
  expect(sim::EngineTestAccess::run_search_items_one_step(engine),
         "The recorded T4 continuation did not play Earthen Vessel.");
  expect(std::count(sim::EngineTestAccess::state(engine).discard.begin(),
                    sim::EngineTestAccess::state(engine).discard.end(),
                    sim::Card::DialgaGX) == 1,
         "The recorded T4 continuation did not discard the held Dragon payload.");
}
...
int main() {
  ...
  test_recorded_t4_continuation_plays_vessel();
