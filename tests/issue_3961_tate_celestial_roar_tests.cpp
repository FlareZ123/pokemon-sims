...
void test_tate_switch_preserves_vstar_and_enables_celestial_roar() {
  ...
  // On the confirmed first turn going second, the switch preserves the known VSTAR
  // and keeps the immediate legal attack instead of shuffling that setup piece away.
...
void test_tate_route_yields_to_later_turn_latias_connector() {
  ...
  // #3961's first-turn exception must not consume Tate before that stronger route.
