...
void test_paid_tapu_preserves_same_turn_burnet_finish() {
  const sim::Scenario scenario{"issue-4157-exact", sim::DciProfile::MatchupFlexJit,
                               sim::LockMode::None, true, 5};
  std::mt19937_64 rng{415700};
  sim::Engine engine = make_engine(scenario, rng, route_state());

  // Retreat and Supporter are distinct turn actions. Paying Tapu's banked Energy
  // keeps Professor Burnet available, whereas Tate & Liza consumes the Supporter.
  ...
  expect(engine.state().active && engine.state().active->card == sim::Card::RegidragoVstar,
  ...
  expect(engine.state().supporter_used && sim::EngineTestAccess::payload_ready(engine),
         "Professor Burnet did not complete the preserved same-turn payload axis.");
}

void test_tate_keeps_priority_without_live_burnet_finish() {
  const sim::Scenario scenario{"issue-4157-controls", sim::DciProfile::MatchupFlexJit,
                               sim::LockMode::None, true, 5};
  sim::State no_burnet = route_state();
  no_burnet.hand.erase(std::remove(no_burnet.hand.begin(), no_burnet.hand.end(),
                                   sim::Card::ProfessorBurnet), no_burnet.hand.end());
  std::mt19937_64 rng{415701};
  sim::Engine engine = make_engine(scenario, rng, std::move(no_burnet));
  expect(!sim::EngineTestAccess::preserves_burnet_finish(engine) &&
             !sim::EngineTestAccess::paid_tapu_available(engine),
  ...
  std::mt19937_64 rng2{415702};
  sim::Engine engine2 = make_engine(scenario, rng2, std::move(no_payload));
  expect(!sim::EngineTestAccess::preserves_burnet_finish(engine2) &&
             !sim::EngineTestAccess::paid_tapu_available(engine2),
         "Tate & Liza should keep priority when Burnet cannot complete payload.");
}

void test_action_and_free_retreat_boundaries() {
  const sim::Scenario scenario{"issue-4157-boundaries", sim::DciProfile::MatchupFlexJit,
                               sim::LockMode::None, true, 5};
  sim::State no_energy = route_state();
  no_energy.active->grass = 0;
  no_energy.active->fire = 0;
  std::mt19937_64 rng{415703};
  sim::Engine no_energy_engine = make_engine(scenario, rng, std::move(no_energy));
  expect(!sim::EngineTestAccess::paid_tapu_available(no_energy_engine),
  ...
  std::mt19937_64 rng2{415704};
  sim::Engine spent_engine = make_engine(scenario, rng2, std::move(spent_retreat));
  expect(!sim::EngineTestAccess::preserves_burnet_finish(spent_engine),
  ...
  std::mt19937_64 rng3{415705};
  sim::Engine free_engine = make_engine(scenario, rng3, std::move(free_retreat));
...
int main() {
  ...
  test_action_and_free_retreat_boundaries();
  return 0;
}