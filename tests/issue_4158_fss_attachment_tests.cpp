...
void test_non_v_holder_is_physically_legal_and_safe_after_vstar_use() {
  ...
  // With the one-per-game VSTAR Power already spent, non-V hand-thinning cannot
  // sacrifice future Star Alchemy value:
  ...
  // Strategy/card/rules separation: https://github.com/FlareZ123/pokemon-sims/blob/main/CARD_CLASS_CLEANUP.md
...
void test_v_holder_remains_strategy_preferred() {
  ...
  // Physical legality is broad while strategy still prefers a Pokémon V holder
  // because only that holder can use the VSTAR Power printed on Forest Seal Stone:
...
void test_bench_non_v_is_physically_legal_when_active_is_disallowed() {
  ...
  // This probes physical holder legality only. Strategy may still preserve FSS
  // for a Pokémon V while Star Alchemy remains available:
  // Forest Seal Stone: https://api.pokemontcg.io/v2/cards/swsh12-156
