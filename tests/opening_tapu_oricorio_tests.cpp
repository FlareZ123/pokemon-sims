...
void test_duplicate_crispin_contention_preserves_vital_dance() {
  ...
  // Fire Energy alone does not complete Regidrago VSTAR's GGF requirement. With
  // duplicate Crispin plus Arven competing for the single Supporter play, keeping
  // Oricorio in hand preserves the zero-Supporter Vital Dance Energy route:
  // https://tcg.pokemon.com/assets/img/learn-to-play/getting-started/quick-start-rules/en-us/quick_start_rulebook.pdf#Set_Up_to_Play
  // https://api.pokemontcg.io/v2/cards/sm2-55
  // https://api.pokemontcg.io/v2/cards/cel25c-60_A
  // https://api.pokemontcg.io/v2/cards/sv7-133
  ...
  if (!after.active || after.active->card != sim::Card::TapuLeleGX ||
    ...
    throw std::runtime_error(
        "Duplicate Crispin and Arven contention should preserve Vital Dance.");
  }
}
...
void test_both_energy_types_preserve_wonder_tag() {
  ...
  if (!after.active || after.active->card != sim::Card::Oricorio ||
      !contains(after.hand, sim::Card::TapuLeleGX)) {
    throw std::runtime_error(
        "Holding both required Energy types should preserve Wonder Tag.");
  }
}

void test_unique_wonder_tag_connector_remains_live() {
  ...
  if (!after.active || after.active->card != sim::Card::Oricorio ||
      !contains(after.hand, sim::Card::TapuLeleGX)) {
    throw std::runtime_error(
        "Wonder Tag should remain live when no held Supporter route replaces it.");
  }
}
...
int main() {
  ...
  test_duplicate_crispin_contention_preserves_vital_dance();
  test_both_energy_types_preserve_wonder_tag();
  test_unique_wonder_tag_connector_remains_live();
