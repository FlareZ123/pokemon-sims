...
void test_registry_metadata_and_supporter_classification() {
  ...
  require(definition != nullptr,
          "Steven's Resolve must be explicitly registered.");
  ...
  require(definition->canonical_id == "sm7-145",
          "Steven's Resolve canonical print changed."); // Card data: https://api.pokemontcg.io/v2/cards/sm7-145
  require(definition->name == "Steven's Resolve",
          "Steven's Resolve display name changed.");
  require(definition->kind == sim::cards::CardKind::Trainer,
          "Steven's Resolve must remain a Trainer."); // Supporter procedure: https://github.com/FlareZ123/pokemon-sims/blob/main/EN_advanced_manual-2025-transcription-structured.md
  require(definition->trainer_kind == sim::cards::TrainerKind::Supporter,
          "Steven's Resolve must remain a Supporter."); // Card data: https://api.pokemontcg.io/v2/cards/sm7-145
  ...
  require(!sim::is_item(sim::Card::StevensResolve),
          "Steven's Resolve must not be classified as an Item.");
