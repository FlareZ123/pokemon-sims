...
LOCK_PATH = Path(".issue-837.lock")


def atomic_replace(path: Path, text: str) -> None:
    temporary = path.with_name(f".{path.name}.issue-837.tmp")
...
def replace_once(path: Path, old: str, new: str) -> None:
    ...
    if new in text:
        return
    if old not in text:
        raise SystemExit(f"issue 837 anchor missing in {path}")
...
def create_exact(path: Path, content: str) -> None:
    if path.exists():
        if path.read_text(encoding="utf-8") == content:
            return
        raise SystemExit(f"issue 837 file already exists with different content: {path}")
...
def main() -> None:
    ...
    try:
        ...
        source = Path("src/trace_engine_v2/part_roseanne_multimode_override.inc")
...
'''
      ...
      if (!remove_one(simulated.state_.hand, Card::RoseannesBackup) ||
          !remove_one(simulated.state_.discard, Card::RegidragoVstar) ||
          !remove_one(simulated.state_.discard, needed_energy)) {
        return false;
      }
      simulated.state_.deck.push_back(Card::RegidragoVstar);
      simulated.state_.deck.push_back(needed_energy);
      simulated.state_.discard.push_back(Card::RoseannesBackup);
      simulated.state_.supporter_used = true;
      // Roseanne's Backup moves each selected card from discard into the deck. Keep
      // the copied zones exact before Pokémon Communication evaluates the restored
      // VSTAR and before Earthen Vessel reserves a separate discard cost:
...
'''
        ...
        test_file = Path("tests/roseanne_pokemon_communication_zone_tests.cpp")
...
'''
        create_exact(test_file, test_content)

        cmake = Path("CMakeLists.txt")
        append_once(
            cmake,
            "regidrago_roseanne_pokemon_communication_zone_tests",
            '''# Roseanne's Backup -> Pokémon Communication -> Earthen Vessel regression:
# https://api.pokemontcg.io/v2/cards/swsh9-148
# https://api.pokemontcg.io/v2/cards/sm9-152
# https://api.pokemontcg.io/v2/cards/sv4-163
# https://github.com/FlareZ123/pokemon-sims/issues/837
add_executable(regidrago_roseanne_pokemon_communication_zone_tests
  tests/roseanne_pokemon_communication_zone_tests.cpp)
target_compile_options(regidrago_roseanne_pokemon_communication_zone_tests
  PRIVATE -Wall -Wextra -Wpedantic -Wconversion -Wshadow)
add_test(NAME regidrago_roseanne_pokemon_communication_zones
  COMMAND regidrago_roseanne_pokemon_communication_zone_tests)''',
        )
    finally:
        os.close(descriptor)
