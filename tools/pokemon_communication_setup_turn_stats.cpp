...
TurnStats simulate_turns(const sim::Scenario& scenario,
  ...
  for (std::uint64_t trial = 0; trial < kTrials; ++trial) {
    ...
    if (ready_turn >= 2 && ready_turn <= 4) {
      const auto turn_index = static_cast<std::size_t>(ready_turn);
      ++stats.exact[turn_index];
      const long double turn = static_cast<long double>(ready_turn);
      stats.ready_turn_sum += turn;
      stats.ready_turn_square_sum += turn * turn;
      stats.censored_turn_sum += turn;
      stats.censored_turn_square_sum += turn * turn;
    } else {
      ++stats.exact[0];
      // The engine ends after T4. Coding a miss as 5 gives E[min(T, 5)], a
      ...
      stats.censored_turn_sum += 5.0L;
      stats.censored_turn_square_sum += 25.0L;
    }

    stats.total_mulligans += static_cast<std::uint64_t>(outcome.mulligans);
...
double mean_from_sum(const long double sum, const std::uint64_t count) {
  return count == 0 ? 0.0 :
      static_cast<double>(sum / static_cast<long double>(count));
}

double sample_mean_se(const long double sum,
                      const long double square_sum,
                      const std::uint64_t count) {
  if (count < 2) return 0.0;
  const long double n = static_cast<long double>(count);
  long double variance = (square_sum - sum * sum / n) / (n - 1.0L);
...
void print_row(const std::string_view label, const TurnStats& stats) {
  ...
  const std::uint64_t by3 = by2 + stats.exact[3];
  const std::uint64_t by4 = by3 + stats.exact[4];

  std::cout << label << ',' << stats.trials << ','
            << stats.exact[2] << ',' << stats.exact[3] << ','
            << stats.exact[4] << ',' << stats.exact[0] << ','
            << pct(stats.exact[2], stats.trials) << ','
            << pct(stats.exact[3], stats.trials) << ','
            << pct(stats.exact[4], stats.trials) << ','
            << pct(stats.exact[0], stats.trials) << ','
            << pct(by2, stats.trials) << ','
            << pct(by3, stats.trials) << ','
            << pct(by4, stats.trials) << ','
            << mean_from_sum(stats.ready_turn_sum, ready_count) << ','
            << sample_mean_se(stats.ready_turn_sum,
                              stats.ready_turn_square_sum, ready_count) << ','
            << mean_from_sum(stats.censored_turn_sum, stats.trials) << ','
            << sample_mean_se(stats.censored_turn_sum,
                              stats.censored_turn_square_sum, stats.trials) << ','
            << quantile_code(stats, 0.50) << ','
            << quantile_code(stats, 0.75) << ','
            << quantile_code(stats, 0.90) << ','
            << mean_from_sum(static_cast<long double>(stats.total_mulligans),
                             stats.trials) << ','
            << pct(stats.started_regi, stats.trials) << ','
