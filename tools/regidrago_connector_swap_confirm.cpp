...
int main(int argc, char** argv) {
  ...
  const std::filesystem::path output_dir = argc >= 3
      ...
      : std::filesystem::path("results/connector-swap-confirm");
  if (trials == 0) return 2;

  const auto must_swap = [](const sim::Card cut, const sim::Card add) {
    const auto variant = make_swap(cut, add);
    if (!variant) throw std::logic_error("confirmation swap is not a legal 60-card recipe");
    return *variant;
  };

  // High-sample confirmation includes the strongest VSTAR-capable compression
  // candidates, Battle VIP Pass as a Basic-compression candidate, Quick Ball as
  // an explicit Basic-only control, and two deliberately unbalanced stress tests.
  ...
  std::vector<Variant> deck_variants{
      ...
      must_swap(sim::Card::ProfessorBurnet, sim::Card::UltraBall),
      must_swap(sim::Card::ProfessorBurnet, sim::Card::RegidragoVstar),
      must_swap(sim::Card::ProfessorBurnet, sim::Card::EvolutionIncense),
      must_swap(sim::Card::ProfessorBurnet, sim::Card::BattleVipPass),
      must_swap(sim::Card::ProfessorBurnet, sim::Card::QuickBall),
      must_swap(sim::Card::ProfessorBurnet, sim::Card::Arven),
      must_swap(sim::Card::Gladion, sim::Card::Arven),
      must_swap(sim::Card::Gladion, sim::Card::BattleVipPass),
      must_swap(sim::Card::Gladion, sim::Card::EvolutionIncense),
      must_swap(sim::Card::ForestSealStone, sim::Card::EvolutionIncense),
      must_swap(sim::Card::Oricorio, sim::Card::RegidragoVstar),
      must_swap(sim::Card::BrilliantBlender, sim::Card::SecretBox),
  };
  ...
  std::cout << "confirmed " << deck_variants.size() << " variants x "
            << scenarios.size() << " scenarios x " << trials << " paired trials\n";
  return 0;
}
