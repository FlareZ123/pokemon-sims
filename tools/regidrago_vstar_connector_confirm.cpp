...
int main(int argc, char** argv) {
  ...
  const std::filesystem::path output_dir = argc >= 3
      ...
      : std::filesystem::path("results/vstar-connector-confirm");
  if (trials == 0) return 2;

  const auto must_swap = [](const sim::Card cut, const sim::Card add) {
    const auto variant = make_swap(cut, add);
    if (!variant) throw std::logic_error("confirmation swap is not legal");
    return *variant;
  };

  // Keep connector/action classes as comparable as possible while testing the
  // actual VSTAR bottleneck. Ultra Ball can search Regidrago VSTAR; Quick Ball and
  // Battle VIP Pass cannot. Evolution Incense can search the VSTAR but not a Basic.
  ...
  std::vector<Variant> deck_variants{
      ...
      must_swap(sim::Card::HisuianHeavyBall, sim::Card::UltraBall),
      must_swap(sim::Card::HisuianHeavyBall, sim::Card::EvolutionIncense),
      must_swap(sim::Card::HisuianHeavyBall, sim::Card::RegidragoVstar),
      must_swap(sim::Card::Gladion, sim::Card::UltraBall),
      must_swap(sim::Card::TapuLeleGX, sim::Card::UltraBall),
      must_swap(sim::Card::QuickBall, sim::Card::UltraBall),
      must_swap(sim::Card::QuickBall, sim::Card::EvolutionIncense),
      must_swap(sim::Card::EarthenVessel, sim::Card::UltraBall),
  };
